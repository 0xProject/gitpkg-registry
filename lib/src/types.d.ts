...
import { ERC20BridgeSource, GetMarketOrdersOpts, OptimizedMarketOrder, TokenAdjacencyGraph } from './utils/market_operation_utils/types';
...
export interface RfqtFirmQuoteValidator {
    filterInvalidQuotesAsync(quotes: Array<{
        response: RFQTFirmQuote;
        makerUri: string;
    }>): Promise<RFQTFirmQuote[]>;
}
export interface RfqtIndicativeQuoteObserver {
    onValidQuotes(quotes: Array<{
        response: RFQTIndicativeQuote;
        makerUri: string;
    }>): void;
}
export interface SwapQuoterRfqtOpts {
    ...
    indicativeQuoteObserver?: RfqtIndicativeQuoteObserver;
}
