...
function fromStringifiedSignedOrderV4(order) {
    return Object.assign(Object.assign({}, order), { 
        // tslint:disable-next-line: custom-no-magic-numbers
        chainId: Number.parseInt(order.chainId, 10), makerAmount: new utils_1.BigNumber(order.makerAmount), verifyingContract: order.exchangeAddress, takerAmount: new utils_1.BigNumber(order.takerAmount), takerTokenFeeAmount: new utils_1.BigNumber(order.takerTokenFeeAmount), expiry: new utils_1.BigNumber(order.expiry), signature: {
            signatureType: parseInt(order.signatureType),
            v: parseInt(order.signatureV),
            r: order.signatureR,
            s: order.signatureS,
...
function fromStringifiedOrderEvent(event) {
    return Object.assign(Object.assign({}, event), { timestampMs: new Date(event.timestamp).getUTCMilliseconds(), order: fromStringifiedOrderWithMetadata(event.order), orderv4: fromStringifiedOrderWithMetadataV4(event.orderv4) });
}
