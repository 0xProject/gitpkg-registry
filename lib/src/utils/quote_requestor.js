...
var constants_2 = require("./market_operation_utils/constants");
// tslint:disable-next-line: custom-no-magic-numbers
var KEEP_ALIVE_TTL = 5 * 60 * constants_2.ONE_SECOND_MS;
...
function convertIfAxiosError(error) {
    ...
    if (error.hasOwnProperty('isAxiosError') && error.isAxiosError) {
        var message = error.message, name_1 = error.name, config = error.config;
        var headers = config.headers, timeout = config.timeout, httpsAgent = config.httpsAgent;
        var keepAlive = httpsAgent.keepAlive, keepAliveMsecs = httpsAgent.keepAliveMsecs, sockets = httpsAgent.sockets;
        var socketCounts = {};
        try {
            for (var _b = __values(Object.keys(sockets)), _c = _b.next(); !_c.done; _c = _b.next()) {
                var socket = _c.value;
...
var QuoteRequestor = /** @class */ (function () {
    ...
    QuoteRequestor.prototype.requestRfqtFirmQuotesAsync = function (makerAssetData, takerAssetData, assetFillAmount, marketOperation, options) {
        return __awaiter(this, void 0, void 0, function () {
            var _opts, firmQuoteResponses, result;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        firmQuoteResponses = _a.sent();
                        result = [];
                        firmQuoteResponses.forEach(function (firmQuoteResponse) {
                            var orderWithStringInts = firmQuoteResponse.response.signedOrder;
                            try {
                                var hasValidSchema = _this._schemaValidator.isValid(orderWithStringInts, json_schemas_1.schemas.signedOrderSchema);
                                if (!hasValidSchema) {
                                    throw new Error('Order not valid');
                                }
                            }
                            catch (err) {
                                _this._warningLogger(orderWithStringInts, "Invalid RFQ-t order received, filtering out. " + err.message);
                                return;
                            }
                            if (!hasExpectedAssetData(makerAssetData, takerAssetData, orderWithStringInts.makerAssetData.toLowerCase(), orderWithStringInts.takerAssetData.toLowerCase())) {
                                _this._warningLogger(orderWithStringInts, 'Unexpected asset data in RFQ-T order, filtering out');
                                return;
                            }
                            if (orderWithStringInts.takerAddress.toLowerCase() !== _opts.takerAddress.toLowerCase()) {
                                _this._warningLogger(orderWithStringInts, 'Unexpected takerAddress in RFQ-T order, filtering out');
                                return;
                            }
                            var orderWithBigNumberInts = __assign({}, orderWithStringInts, { makerAssetAmount: new utils_1.BigNumber(orderWithStringInts.makerAssetAmount), takerAssetAmount: new utils_1.BigNumber(orderWithStringInts.takerAssetAmount), makerFee: new utils_1.BigNumber(orderWithStringInts.makerFee), takerFee: new utils_1.BigNumber(orderWithStringInts.takerFee), expirationTimeSeconds: new utils_1.BigNumber(orderWithStringInts.expirationTimeSeconds), salt: new utils_1.BigNumber(orderWithStringInts.salt) });
                            if (order_utils_1.orderCalculationUtils.willOrderExpire(orderWithBigNumberInts, _this._expiryBufferMs / constants_1.constants.ONE_SECOND_MS)) {
                                _this._warningLogger(orderWithBigNumberInts, 'Expiry too soon in RFQ-T order, filtering out');
                                return;
                            }
                            // Store makerUri for looking up later
                            _this._orderHashToMakerUri[order_utils_1.orderHashUtils.getOrderHash(orderWithBigNumberInts)] = firmQuoteResponse.makerUri;
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        validResponsesWithStringInts = responsesWithStringInts.filter(function (result) {
                        ...
                        validResponses = validResponsesWithStringInts.map(function (result) {
                            var response = result.response;
     ...
     */
    QuoteRequestor.prototype.getMakerUriForOrderHash = function (orderHash) {
                ...
                switch (_a.label) {
                    case 0:
                        ...
                        return [4 /*yield*/, Promise.all(Object.keys(this._rfqtAssetOfferings).map(function (url) { return __awaiter(_this, void 0, void 0, function () {
                                var requestParamsWithBigNumbers, requestParams, partialLogEntry, timeBeforeAwait, quotePath, response, err_1;
                                return __generator(this, function (_a) {
                                    switch (_a.label) {
                                        case 0:
                                            if (!this._makerSupportsPair(url, makerAssetData, takerAssetData)) return [3 /*break*/, 4];
                                            requestParamsWithBigNumbers = __assign({ takerAddress: options.takerAddress }, inferQueryParams(marketOperation, makerAssetData, takerAssetData, assetFillAmount));
                                            requestParams = __assign({}, requestParamsWithBigNumbers, { sellAmountBaseUnits: requestParamsWithBigNumbers.sellAmountBaseUnits
                                                    ? requestParamsWithBigNumbers.sellAmountBaseUnits.toString()
                                                    : undefined, buyAmountBaseUnits: requestParamsWithBigNumbers.buyAmountBaseUnits
                                        ...
                                        case 1:
                                            ...
                                            return [4 /*yield*/, exports.quoteRequestorHttpClient.get(url + "/" + quotePath, {
                                        ...
                                        case 3:
                                            ...
                                            this._infoLogger({
                                                rfqtMakerInteraction: __assign({}, partialLogEntry, { response: {
                                                        included: false,
                                                        statusCode: err_1.response ? err_1.response.status : undefined,
                                                        latencyMs: Date.now() - timeBeforeAwait,
                                                    } }),
                                            });
