                ...
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        validatedOrdersWithStringInts = ordersWithStringInts.filter(function (order) {
                            ...
                            var minExpiry = Math.round((Date.now() + _this._expiryBufferMs) / constants_1.constants.ONE_SECOND_MS);
                            if (new utils_1.BigNumber(order.expirationTimeSeconds).isLessThan(minExpiry)) {
                                _this._warningLogger("Expiry too soon (expected at least " + minExpiry + ") in RFQ-T order, filtering out: " + JSON.stringify(order));
                                return false;
                            }
                            return true;
                        });
                ...
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        validResponsesWithStringInts = responsesWithStringInts.filter(function (response) {
                            ...
                            var minExpiry = Math.round((Date.now() + _this._expiryBufferMs) / constants_1.constants.ONE_SECOND_MS);
                            if (new utils_1.BigNumber(response.expirationTimeSeconds).isLessThan(minExpiry)) {
                                _this._warningLogger("Expiry too soon (expected at least " + minExpiry + ") in RFQ-T indicative quote, filtering out: " + JSON.stringify(response));
                                return false;
                            }
                            return true;
                        });
