                ...
                if (isBlacklisted) {
                    ...
                    try {
                        if (typedMakerUrl.pairType === types_1.RfqPairType.Standard) {
                            ...
                            const quote = yield alt_mm_implementation_utils_1.returnQuoteFromAltMMAsync(typedMakerUrl.url, this._altRfqCreds.altRfqApiKey, this._altRfqCreds.altRfqProfile, options.apiKey, quoteType === 'firm' ? types_1.AltQuoteModel.Firm : types_1.AltQuoteModel.Indicative, makerToken, takerToken, timeoutMs, options.altRfqtAssetOfferings || {}, requestParams, this._quoteRequestorHttpClient, cancelTokenSource.token);
                            const latencyMs = Date.now() - timeBeforeAwait;
                            this._infoLogger({
                                rfqtMakerInteraction: Object.assign({}, partialLogEntry, { response: {
                                        included: true,
