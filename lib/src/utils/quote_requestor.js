                ...
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        validatedOrdersWithStringInts = ordersWithStringInts.filter(function (order) {
                            ...
                            var maxExpiry = Math.round((Date.now() + _this._expiryBufferMs) / constants_1.constants.ONE_SECOND_MS);
                            if (new utils_1.BigNumber(order.expirationTimeSeconds).isGreaterThan(maxExpiry)) {
                                _this._warningLogger("Expiry too soon (expected <= " + maxExpiry + ") in RFQ-T order, filtering out: " + JSON.stringify(order));
                                return false;
                            }
                            return true;
                        });
                ...
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        validResponsesWithStringInts = responsesWithStringInts.filter(function (response) {
                            ...
                            var maxExpiry = Math.round((Date.now() + _this._expiryBufferMs) / constants_1.constants.ONE_SECOND_MS);
                            if (new utils_1.BigNumber(response.expirationTimeSeconds).isGreaterThan(maxExpiry)) {
                                _this._warningLogger("Expiry too soon (expected <=" + maxExpiry + ")in RFQ-T indicative quote, filtering out: " + JSON.stringify(response));
                                return false;
                            }
                            return true;
                        });
