        ...
        if (didTimeout) {
            ...
            if (this._makerTimeoutStreakLength[makerUrl] === this._timeoutStreakThreshold) {
                this._makerBlacklistedUntilDate[makerUrl] =
                    Date.now() + this._blacklistDurationMinutes * constants_1.constants.ONE_MINUTE_MS;
                this.infoLogger({ makerUrl, blacklistedUntil: new Date(this._makerBlacklistedUntilDate[makerUrl]).toISOString() }, 'maker blacklisted');
            }
        }
        else {
            this._makerTimeoutStreakLength[makerUrl] = 0;
