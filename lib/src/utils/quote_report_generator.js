 ...
 */
function generateQuoteReport(marketOperation, dexQuotes, multiHopQuotes, nativeOrders, orderFillableAmounts, liquidityDelivered, comparisonPrice, quoteRequestor) {
    const dexReportSourcesConsidered = dexQuotes.map(quote => _dexSampleToReportSource(quote, marketOperation));
    const nativeOrderSourcesConsidered = nativeOrders.map((order, idx) => _nativeOrderToReportSource(order, orderFillableAmounts[idx], comparisonPrice, quoteRequestor));
    ...
    if (Array.isArray(liquidityDelivered)) {
        ...
        sourcesDelivered = liquidityDelivered.map(collapsedFill => {
            ...
            if (foundNativeOrder) {
                return _nativeOrderToReportSource(foundNativeOrder, nativeOrderSignaturesToFillableAmounts[foundNativeOrder.signature], comparisonPrice, quoteRequestor);
            }
            else {
...
function _nativeOrderToReportSource(nativeOrder, fillableAmount, comparisonPrice, quoteRequestor) {
