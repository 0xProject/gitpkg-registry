...
exports.indicativeQuoteToReportEntry = exports.nativeOrderToReportEntry = exports.multiHopSampleToReportSource = exports.dexSampleToReportSource = exports.generateExtendedQuoteReport = exports.generateQuoteReport = void 0;
 ...
 */
function generateExtendedQuoteReport(marketOperation, quotes, liquidityDelivered, comparisonPrice, quoteRequestor) {
    let sourcesConsidered = [];
    // NativeOrders
    sourcesConsidered.push(...quotes.nativeOrders.map(order => nativeOrderToReportEntry(order.type, order, order.fillableTakerAmount, comparisonPrice, quoteRequestor)));
    // IndicativeQuotes
    ...
    // Dex Quotes
    sourcesConsidered.push(..._.flatten(quotes.dexQuotes.map(dex => dex.map(quote => dexSampleToReportSource(quote, marketOperation)))));
    let sourcesDelivered;
    if (Array.isArray(liquidityDelivered)) {
        // create easy way to look up fillable amounts
...
exports.generateExtendedQuoteReport = generateExtendedQuoteReport;
...
function nativeOrderToReportEntry(type, fillData, fillableAmount, comparisonPrice, quoteRequestor) {
    ...
    const isRFQ = type === protocol_utils_1.FillQuoteTransformerOrderType.Rfq;
    ...
    if (isRFQ) {
        ...
        return Object.assign(Object.assign(Object.assign(Object.assign({ liquiditySource: types_2.ERC20BridgeSource.Native }, nativeOrderBase), { isRFQ: true, makerUri: rfqtMakerUri || '' }), (comparisonPrice ? { comparisonPrice: comparisonPrice.toNumber() } : {})), { nativeOrder,
            fillData });
    }
    else {
        // tslint:disable-next-line: no-object-literal-type-assertion
        return Object.assign(Object.assign({ liquiditySource: types_2.ERC20BridgeSource.Native }, nativeOrderBase), { isRFQ: false, fillData });
    }
}
