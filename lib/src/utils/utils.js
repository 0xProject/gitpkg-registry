...
function timeItAsync(cb, handler, threshold = 100) {
    return __awaiter(this, void 0, void 0, function* () {
        let dt = Date.now();
        const r = yield cb();
        dt = Date.now() - dt;
        if (dt >= threshold) {
            const msg = (handler || ((..._args) => { }))(dt);
            console.debug(`[DEBUG] ${msg}`);
        }
        return r;
    });
}
exports.timeItAsync = timeItAsync;
function timeIt(cb, handler, threshold = 100) {
    let dt = Date.now();
    const r = cb();
    dt = Date.now() - dt;
    if (dt >= threshold) {
