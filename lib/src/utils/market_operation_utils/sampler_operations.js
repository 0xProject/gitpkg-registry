...
class SamplerOperations {
    ...
    getSellQuotes(sources, makerToken, takerToken, takerFillAmounts, wethAddress, liquidityProviderRegistryAddress, multiBridgeAddress) {
        ...
        const subOps = this._getSellQuoteOperations(_sources, makerToken, takerToken, takerFillAmounts, wethAddress, liquidityProviderRegistryAddress, multiBridgeAddress);
        return {
            encodeCall: () => {
                const subCalls = subOps.map(op => op.encodeCall());
                return this._samplerContract.batchCall(subCalls).getABIEncodedTransactionData();
    ...
    getBuyQuotes(sources, makerToken, takerToken, makerFillAmounts, wethAddress, liquidityProviderRegistryAddress) {
        const _sources = exports.BATCH_SOURCE_FILTERS.getAllowed(sources);
        const subOps = this._getBuyQuoteOperations(_sources, makerToken, takerToken, makerFillAmounts, wethAddress, liquidityProviderRegistryAddress);
        return {
            encodeCall: () => {
                const subCalls = subOps.map(op => op.encodeCall());
                return this._samplerContract.batchCall(subCalls).getABIEncodedTransactionData();
