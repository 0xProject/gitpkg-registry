        ...
        const allOps = _.flatten(_sources.map((source) => {
            ...
            switch (source) {
                ...
                case types_1.ERC20BridgeSource.Clipper:
                    const { poolAddress: clipperPoolAddress, tokens: clipperTokens } = constants_1.CLIPPER_INFO_BY_CHAIN[this.chainId];
                    if (clipperPoolAddress === constants_1.NULL_ADDRESS ||
                        !clipperTokens.includes(makerToken) ||
                        !clipperTokens.includes(takerToken)) {
                        return [];
                    }
                    // Clipper requires WETH to be represented as address(0)
                    const adjustedMakerToken = makerToken === constants_1.NATIVE_FEE_TOKEN_BY_CHAIN_ID[this.chainId] ? constants_1.NULL_ADDRESS : makerToken;
                    const adjustedTakerToken = takerToken === constants_1.NATIVE_FEE_TOKEN_BY_CHAIN_ID[this.chainId] ? constants_1.NULL_ADDRESS : takerToken;
                    // Supports the PLP interface
                    return this.getLiquidityProviderSellQuotes(clipperPoolAddress, adjustedMakerToken, adjustedTakerToken, takerFillAmounts, 
        ...
        return _.flatten(_sources.map((source) => {
            switch (source) {
                ...
                case types_1.ERC20BridgeSource.Clipper:
                    ...
                    const adjustedTakerToken = takerToken === constants_1.NATIVE_FEE_TOKEN_BY_CHAIN_ID[this.chainId] ? constants_1.NULL_ADDRESS : takerToken;
                    // Supports the PLP interface
                    return this.getLiquidityProviderBuyQuotes(clipperPoolAddress, adjustedMakerToken, adjustedTakerToken, makerFillAmounts, 
                    // tslint:disable-next-line: custom-no-magic-numbers
                    0, // Not used for Clipper
                    types_1.ERC20BridgeSource.Clipper);
                default:
