...
class SamplerOperations {
    constructor(_samplerContract, provider, balancerPoolsCache = new balancer_utils_1.BalancerPoolsCache(), creamPoolsCache = new cream_utils_1.CreamPoolsCache(), getBancorServiceFn) {
    ...
    getCreamSellQuotesOffChainAsync(makerToken, takerToken, takerFillAmounts) {
        return __awaiter(this, void 0, void 0, function* () {
            const pools = yield this.creamPoolsCache.getPoolsForPairAsync(takerToken, makerToken);
            return pools.map(pool => takerFillAmounts.map(amount => ({
                source: types_1.ERC20BridgeSource.Cream,
                output: balancer_utils_1.computeBalancerSellQuote(pool, amount),
                input: amount,
                fillData: { poolAddress: pool.id },
            })));
    ...
    getTwoHopSellQuotes(sources, makerToken, takerToken, sellAmount, wethAddress, tokenAdjacencyGraph, liquidityProviderRegistryAddress) {
        ...
        const subOps = intermediateTokens.map(intermediateToken => {
            const firstHopOps = this._getSellQuoteOperations(_sources, intermediateToken, takerToken, [constants_1.ZERO_AMOUNT], wethAddress, tokenAdjacencyGraph, liquidityProviderRegistryAddress);
            const secondHopOps = this._getSellQuoteOperations(_sources, makerToken, intermediateToken, [constants_1.ZERO_AMOUNT], wethAddress, tokenAdjacencyGraph, liquidityProviderRegistryAddress);
            return new sampler_contract_operation_1.SamplerContractOperation({
                contract: this._samplerContract,
    ...
    getTwoHopBuyQuotes(sources, makerToken, takerToken, buyAmount, wethAddress, tokenAdjacencyGraph, liquidityProviderRegistryAddress) {
        const _sources = exports.TWO_HOP_SOURCE_FILTERS.getAllowed(sources);
        if (_sources.length === 0) {
            return SamplerOperations.constant([]);
        }
        const intermediateTokens = multihop_utils_1.getIntermediateTokens(makerToken, takerToken, tokenAdjacencyGraph, wethAddress);
        const subOps = intermediateTokens.map(intermediateToken => {
            const firstHopOps = this._getBuyQuoteOperations(_sources, intermediateToken, takerToken, [new utils_1.BigNumber(0)], wethAddress, tokenAdjacencyGraph, // TODO is this a bad idea?
            liquidityProviderRegistryAddress);
            const secondHopOps = this._getBuyQuoteOperations(_sources, makerToken, intermediateToken, [new utils_1.BigNumber(0)], wethAddress, tokenAdjacencyGraph, liquidityProviderRegistryAddress);
            return new sampler_contract_operation_1.SamplerContractOperation({
                contract: this._samplerContract,
                source: types_1.ERC20BridgeSource.MultiHop,
    ...
    getShellSellQuotes(poolAddress, makerToken, takerToken, takerFillAmounts) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            params: [poolAddress, takerToken, makerToken, takerFillAmounts],
            callback: (callResults, fillData) => {
                const samples = this._samplerContract.getABIDecodedReturnData('sampleSellsFromShell', callResults);
                fillData.poolAddress = poolAddress;
                return samples;
            },
        });
    ...
    getShellBuyQuotes(poolAddress, makerToken, takerToken, makerFillAmounts) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            params: [poolAddress, takerToken, makerToken, makerFillAmounts],
            callback: (callResults, fillData) => {
                const samples = this._samplerContract.getABIDecodedReturnData('sampleBuysFromShell', callResults);
                fillData.poolAddress = poolAddress;
                return samples;
            },
        });
    ...
    getMedianSellRate(sources, makerToken, takerToken, takerFillAmount, liquidityProviderRegistryAddress, multiBridgeAddress) {
        if (makerToken.toLowerCase() === takerToken.toLowerCase()) {
            return SamplerOperations.constant(new utils_1.BigNumber(1));
        }
        const getSellQuotes = this.getSellQuotes(sources, makerToken, takerToken, [takerFillAmount], utils_1.NULL_ADDRESS, // weth address
        {}, // token adjacency
        liquidityProviderRegistryAddress, multiBridgeAddress);
        return {
            encodeCall: () => {
    ...
    getSellQuotes(sources, makerToken, takerToken, takerFillAmounts, wethAddress, tokenAdjacencyGraph, liquidityProviderRegistryAddress, multiBridgeAddress) {
        const subOps = this._getSellQuoteOperations(sources, makerToken, takerToken, takerFillAmounts, wethAddress, tokenAdjacencyGraph, liquidityProviderRegistryAddress, multiBridgeAddress);
        return {
            encodeCall: () => {
                const subCalls = subOps.map(op => op.encodeCall());
                return this._samplerContract.batchCall(subCalls).getABIEncodedTransactionData();
    ...
    getBuyQuotes(sources, makerToken, takerToken, makerFillAmounts, wethAddress, tokenAdjacencyGraph, liquidityProviderRegistryAddress) {
        const subOps = this._getBuyQuoteOperations(sources, makerToken, takerToken, makerFillAmounts, wethAddress, tokenAdjacencyGraph, liquidityProviderRegistryAddress);
        return {
            encodeCall: () => {
                const subCalls = subOps.map(op => op.encodeCall());
                return this._samplerContract.batchCall(subCalls).getABIEncodedTransactionData();
    ...
    _getSellQuoteOperations(sources, makerToken, takerToken, takerFillAmounts, wethAddress, tokenAdjacencyGraph, liquidityProviderRegistryAddress, multiBridgeAddress) {
        const _sources = exports.BATCH_SOURCE_FILTERS.exclude(liquidityProviderRegistryAddress ? [] : [types_1.ERC20BridgeSource.LiquidityProvider])
            .exclude(multiBridgeAddress || multiBridgeAddress === utils_1.NULL_ADDRESS ? [] : [types_1.ERC20BridgeSource.MultiBridge])
            .getAllowed(sources);
    ...
    _getBuyQuoteOperations(sources, makerToken, takerToken, makerFillAmounts, wethAddress, tokenAdjacencyGraph, liquidityProviderRegistryAddress) {
        const _sources = exports.BATCH_SOURCE_FILTERS.exclude(liquidityProviderRegistryAddress ? [] : [types_1.ERC20BridgeSource.LiquidityProvider]).getAllowed(sources);
        // Find the adjacent tokens in the provided tooken adjacency graph,
        // e.g if this is DAI->USDC we may check for DAI->WETH->USDC
        const intermediateTokens = multihop_utils_1.getIntermediateTokens(makerToken, takerToken, tokenAdjacencyGraph, wethAddress);
        return _.flatten(_sources.map((source) => {
