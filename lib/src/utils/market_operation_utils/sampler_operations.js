...
class SamplerOperations {
    ...
    getTwoHopSellQuotes(sources, makerToken, takerToken, sellAmount, tokenAdjacencyGraph, wethAddress, liquidityProviderRegistryAddress) {
        ...
        const subOps = intermediateTokens.map(intermediateToken => {
            const firstHopOps = this._getSellQuoteOperations(_sources, intermediateToken, takerToken, [constants_1.ZERO_AMOUNT], wethAddress, liquidityProviderRegistryAddress);
            const secondHopOps = this._getSellQuoteOperations(_sources, makerToken, intermediateToken, [constants_1.ZERO_AMOUNT], wethAddress, liquidityProviderRegistryAddress);
            return new sampler_contract_operation_1.SamplerContractOperation({
                contract: this._samplerContract,
    ...
    getTwoHopBuyQuotes(sources, makerToken, takerToken, buyAmount, tokenAdjacencyGraph, wethAddress, liquidityProviderRegistryAddress) {
        ...
        const intermediateTokens = multihop_utils_1.getIntermediateTokens(makerToken, takerToken, tokenAdjacencyGraph, wethAddress);
        const subOps = intermediateTokens.map(intermediateToken => {
            const firstHopOps = this._getBuyQuoteOperations(_sources, intermediateToken, takerToken, [new utils_1.BigNumber(0)], wethAddress, liquidityProviderRegistryAddress);
            const secondHopOps = this._getBuyQuoteOperations(_sources, makerToken, intermediateToken, [new utils_1.BigNumber(0)], wethAddress, liquidityProviderRegistryAddress);
            return new sampler_contract_operation_1.SamplerContractOperation({
                contract: this._samplerContract,
                source: types_1.ERC20BridgeSource.MultiHop,
