...
class SamplerOperations {
    ...
    getBestBuyOutput(sources, makerToken, takerToken, makerFillAmount) {
        if (makerToken.toLowerCase() === takerToken.toLowerCase()) {
            return SamplerOperations.constant(new utils_1.BigNumber(1));
        }
        const subOps = this._getBuyQuoteOperations(sources, makerToken, takerToken, [
            makerFillAmount,
            // upper bound
            makerFillAmount.times(10),
        ]);
        return {
            encodeCall: () => {
                const subCalls = subOps.map(op => op.encodeCall());
            ...
            handleCallResults: callResults => {
                ...
                const bestUpperBound = utils_1.BigNumber.min(...samples.map(s => (s && s.length > 1 && !s[1].isZero() ? s[1] : constants_1.MAX_UINT256)));
                const bestUpperIndex = samples.findIndex(s => s && s.length > 1 && s[1].isEqualTo(bestUpperBound));
                console.log({
                    makerToken,
                    bestUpperBound,
                    samples: subOps.map((s, i) => ({ source: s.source, output: samples[i] && samples[i][0] })),
                    sample: samples[bestUpperIndex] && samples[bestUpperIndex][0],
                });
                const flatSamples = samples.reduce((acc, v) => acc.concat(...v)).filter(v => !v.isZero());
                if (flatSamples.length === 0) {
                    return constants_1.ZERO_AMOUNT;
                }
                return utils_1.BigNumber.min(...flatSamples);
            },
        };
    }
        ...
        return _.flatten(_sources.map((source) => {
            switch (source) {
                ...
                case types_1.ERC20BridgeSource.Mooniswap:
                    return [
                        this.getMooniswapSellQuotes(constants_1.MAINNET_MOONISWAP_REGISTRY, makerToken, takerToken, takerFillAmounts),
                        this.getMooniswapSellQuotes(constants_1.MAINNET_MOONISWAP_V2_REGISTRY, makerToken, takerToken, takerFillAmounts),
                    ];
    ...
    _getBuyQuoteOperations(sources, makerToken, takerToken, makerFillAmounts) {
        ...
        return _.flatten(_sources.map((source) => {
            switch (source) {
                ...
                case types_1.ERC20BridgeSource.Mooniswap:
                    return [
                        this.getMooniswapBuyQuotes(constants_1.MAINNET_MOONISWAP_REGISTRY, makerToken, takerToken, makerFillAmounts),
                        this.getMooniswapBuyQuotes(constants_1.MAINNET_MOONISWAP_V2_REGISTRY, makerToken, takerToken, makerFillAmounts),
                    ];
                case types_1.ERC20BridgeSource.Balancer:
                    return this.balancerPoolsCache
                        .getCachedPoolAddressesForPair(takerToken, makerToken)
