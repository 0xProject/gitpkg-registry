...
const badFillCollector = {};
const isFillDataBad = (source, keys) => {
...
const registerResults = (source, keys, result) => {
    if (!badFillCollector[source]) {
        badFillCollector[source.toString()] = new Collector(source);
    }
...
class SamplerOperations {
    ...
    getKyberSellQuotes(kyberOpts, reserveOffset, makerToken, takerToken, takerFillAmounts) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            callback: (callResults, fillData) => {
                ...
                _log({ source: types_1.ERC20BridgeSource.Kyber, gasUsed: gasUsed.map(g => g.toString(10)) });
    ...
    getKyberDmmSellQuotes(router, tokenAddressPath, takerFillAmounts) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            callback: (callResults, fillData) => {
                ...
                fillData.gasUsed = gasUsed;
                registerResults(types_1.ERC20BridgeSource.KyberDmm, [router, ...tokenAddressPath], {
                    gasUsed,
                    samples,
                });
                _log({ source: types_1.ERC20BridgeSource.KyberDmm, gasUsed: gasUsed.map(g => g.toString(10)) });
                return samples;
            },
        });
    }
    ...
    getUniswapSellQuotes(router, makerToken, takerToken, takerFillAmounts) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleSellsFromUniswap', callResults);
                fillData.gasUsed = gasUsed;
                registerResults(types_1.ERC20BridgeSource.Uniswap, [router, makerToken, takerToken], {
                    gasUsed,
                    samples,
                });
                _log({ source: types_1.ERC20BridgeSource.Uniswap, gasUsed: gasUsed.map(g => g.toString(10)) });
                return samples;
            },
        });
    }
    getUniswapBuyQuotes(router, makerToken, takerToken, makerFillAmounts) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleBuysFromUniswap', callResults);
                fillData.gasUsed = gasUsed;
                _log({ source: types_1.ERC20BridgeSource.Uniswap, gasUsed: gasUsed.map(g => g.toString(10)) });
                return samples;
            },
    ...
    getUniswapV2SellQuotes(router, tokenAddressPath, takerFillAmounts, source = types_1.ERC20BridgeSource.UniswapV2) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            params: [router, tokenAddressPath, takerFillAmounts],
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleSellsFromUniswapV2', callResults);
                fillData.gasUsed = gasUsed;
    ...
    getUniswapV2BuyQuotes(router, tokenAddressPath, makerFillAmounts, source = types_1.ERC20BridgeSource.UniswapV2) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            params: [router, tokenAddressPath, makerFillAmounts],
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleBuysFromUniswapV2', callResults);
                fillData.gasUsed = gasUsed;
                _log({ source, tokenAddressPath, gasUsed: gasUsed.map(g => g.toString(10)) });
                return samples;
            },
    ...
    getLiquidityProviderSellQuotes(providerAddress, makerToken, takerToken, takerFillAmounts, gasCost) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleSellsFromLiquidityProvider', callResults);
                fillData.gasUsed = gasUsed;
                _log({
                    source: types_1.ERC20BridgeSource.LiquidityProvider,
                    fillData,
    ...
    getLiquidityProviderBuyQuotes(providerAddress, makerToken, takerToken, makerFillAmounts, gasCost) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleBuysFromLiquidityProvider', callResults);
                fillData.gasUsed = gasUsed;
                _log({
                    source: types_1.ERC20BridgeSource.LiquidityProvider,
                    fillData,
    ...
    getEth2DaiSellQuotes(router, makerToken, takerToken, takerFillAmounts) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            params: [router, takerToken, makerToken, takerFillAmounts],
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleSellsFromEth2Dai', callResults);
                fillData.gasUsed = gasUsed;
                registerResults(types_1.ERC20BridgeSource.Eth2Dai, [router, makerToken, takerToken], {
                    gasUsed,
                    samples,
                });
                _log({
    ...
    getEth2DaiBuyQuotes(router, makerToken, takerToken, makerFillAmounts) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            params: [router, takerToken, makerToken, makerFillAmounts],
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleBuysFromEth2Dai', callResults);
                fillData.gasUsed = gasUsed;
                _log({
                    source: types_1.ERC20BridgeSource.Eth2Dai,
                    gasUsed: gasUsed.map(g => g.toString(10)),
    ...
    getCurveSellQuotes(pool, makerToken, takerToken, takerFillAmounts, source = types_1.ERC20BridgeSource.Curve) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleSellsFromCurve', callResults);
                fillData.gasUsed = gasUsed;
    ...
    getCurveBuyQuotes(pool, makerToken, takerToken, makerFillAmounts, source = types_1.ERC20BridgeSource.Curve) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleBuysFromCurve', callResults);
                fillData.gasUsed = gasUsed;
                _log({
                    source,
                    pool: pool.poolAddress,
    ...
    getBalancerV2SellQuotes(poolInfo, makerToken, takerToken, takerFillAmounts, source) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            source,
            fillData: Object.assign(Object.assign({}, poolInfo), { gasUsed: [] }),
            contract: this._samplerContract,
            function: this._samplerContract.sampleSellsFromBalancerV2,
            params: [poolInfo, takerToken, makerToken, takerFillAmounts],
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleSellsFromBalancerV2', callResults);
                fillData.gasUsed = gasUsed;
    ...
    getBalancerV2BuyQuotes(poolInfo, makerToken, takerToken, makerFillAmounts, source) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleBuysFromBalancerV2', callResults);
                fillData.gasUsed = gasUsed;
                _log({
                    source,
                    fillData,
    ...
    getBalancerSellQuotes(poolAddress, makerToken, takerToken, takerFillAmounts, source) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleSellsFromBalancer', callResults);
                fillData.gasUsed = gasUsed;
                _log({
                    source,
                    fillData,
    ...
    getBalancerBuyQuotes(poolAddress, makerToken, takerToken, makerFillAmounts, source) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleBuysFromBalancer', callResults);
                fillData.gasUsed = gasUsed;
                _log({
                    source,
                    fillData,
    ...
    getMStableSellQuotes(router, makerToken, takerToken, takerFillAmounts) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            params: [router, takerToken, makerToken, takerFillAmounts],
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleSellsFromMStable', callResults);
                fillData.gasUsed = gasUsed;
                _log({
                    source: types_1.ERC20BridgeSource.MStable,
                    fillData,
    ...
    getMStableBuyQuotes(router, makerToken, takerToken, makerFillAmounts) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            params: [router, takerToken, makerToken, makerFillAmounts],
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleBuysFromMStable', callResults);
                fillData.gasUsed = gasUsed;
                registerResults(types_1.ERC20BridgeSource.MStable, [router, makerToken, takerToken], {
                    gasUsed,
                    samples,
                });
                _log({
    ...
    getBancorSellQuotes(registry, makerToken, takerToken, takerFillAmounts) {
        ...
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            callback: (callResults, fillData) => {
                ...
                fillData.path = path;
                fillData.gasUsed = gasUsed;
                registerResults(types_1.ERC20BridgeSource.Bancor, [registry, makerToken, takerToken], {
                    gasUsed,
                    samples,
                });
                _log({
    ...
    getMooniswapSellQuotes(registry, makerToken, takerToken, takerFillAmounts) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            callback: (callResults, fillData) => {
                ...
                fillData.gasUsed = gasUsed;
                registerResults(types_1.ERC20BridgeSource.Mooniswap, [registry, makerToken, takerToken], {
                    gasUsed,
                    samples,
                });
                _log({
    ...
    getShellSellQuotes(poolAddress, makerToken, takerToken, takerFillAmounts, source = types_1.ERC20BridgeSource.Shell) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            params: [poolAddress, takerToken, makerToken, takerFillAmounts],
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleBuysFromShell', callResults);
                fillData.gasUsed = gasUsed;
    ...
    getDODOSellQuotes(opts, makerToken, takerToken, takerFillAmounts) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            callback: (callResults, fillData) => {
                ...
                fillData.gasUsed = gasUsed;
                registerResults(types_1.ERC20BridgeSource.Dodo, [opts.registry, makerToken, takerToken], {
                    gasUsed,
                    samples,
                });
                _log({
    ...
    getDODOV2SellQuotes(registry, offset, makerToken, takerToken, takerFillAmounts) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            callback: (callResults, fillData) => {
                ...
                fillData.poolAddress = pool;
                fillData.gasUsed = gasUsed;
                registerResults(types_1.ERC20BridgeSource.DodoV2, [registry, offset, makerToken, takerToken], {
                    gasUsed,
                    samples,
                });
                _log({
    ...
    getMakerPsmSellQuotes(psmInfo, makerToken, takerToken, takerFillAmounts) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            fillData: Object.assign(Object.assign({ isSellOperation: true, takerToken,
                makerToken }, psmInfo), { gasUsed: [] }),
            contract: this._samplerContract,
            function: this._samplerContract.sampleSellsFromMakerPsm,
            params: [psmInfo, takerToken, makerToken, takerFillAmounts],
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleSellsFromMakerPsm', callResults);
                fillData.gasUsed = gasUsed;
                registerResults(types_1.ERC20BridgeSource.MakerPsm, [psmInfo.psmAddress, psmInfo.gemTokenAddress, psmInfo.ilkIdentifier, makerToken, takerToken], {
                    gasUsed,
                    samples,
                });
                _log({
    ...
    getMakerPsmBuyQuotes(psmInfo, makerToken, takerToken, makerFillAmounts) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            source: types_1.ERC20BridgeSource.MakerPsm,
            fillData: Object.assign(Object.assign({ isSellOperation: false, takerToken,
                makerToken }, psmInfo), { gasUsed: [] }),
            contract: this._samplerContract,
            function: this._samplerContract.sampleBuysFromMakerPsm,
            params: [psmInfo, takerToken, makerToken, makerFillAmounts],
            callback: (callResults, fillData) => {
                const [gasUsed, samples] = this._samplerContract.getABIDecodedReturnData('sampleBuysFromMakerPsm', callResults);
                fillData.gasUsed = gasUsed;
                _log({
                    source: types_1.ERC20BridgeSource.MakerPsm,
                    fillData,
    ...
    _getBuyQuoteOperations(sources, makerToken, takerToken, makerFillAmounts) {
        ...
        return _.flatten(_sources.map((source) => {
            switch (source) {
                ...
                case types_1.ERC20BridgeSource.XSigma:
                case types_1.ERC20BridgeSource.Smoothy:
                    return bridge_source_utils_1.getCurveLikeInfosForPair(this.chainId, takerToken, makerToken, source).map(pool => this.getCurveBuyQuotes(pool, makerToken, takerToken, makerFillAmounts, source));
                case types_1.ERC20BridgeSource.Shell:
                case types_1.ERC20BridgeSource.Component:
