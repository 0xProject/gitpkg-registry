...
class SamplerOperations {
    ...
    _getSellQuoteOperations(sources, makerToken, takerToken, takerFillAmounts, tokenAdjacencyGraph = this.tokenAdjacencyGraph) {
        ...
        const _sources = constants_1.SELL_SOURCE_FILTER.exclude([types_1.ERC20BridgeSource.MultiHop, types_1.ERC20BridgeSource.Native]).getAllowed(sources);
        const allOps = _.flatten(_sources.map((source) => {
            switch (source) {
                case types_1.ERC20BridgeSource.Eth2Dai:
                    return bridge_source_utils_1.isValidAddress(constants_1.OASIS_ROUTER)
                        ? this.getEth2DaiSellQuotes(constants_1.OASIS_ROUTER, makerToken, takerToken, takerFillAmounts)
                        : [];
                case types_1.ERC20BridgeSource.Uniswap:
                    return bridge_source_utils_1.isValidAddress(constants_1.UNISWAPV1_ROUTER)
                        ? this.getUniswapSellQuotes(constants_1.UNISWAPV1_ROUTER, makerToken, takerToken, takerFillAmounts)
                        : [];
                case types_1.ERC20BridgeSource.UniswapV2:
                    if (!bridge_source_utils_1.isValidAddress(constants_1.UNISWAPV2_ROUTER)) {
                        return [];
                    }
                    const ops = [
                        this.getUniswapV2SellQuotes(constants_1.UNISWAPV2_ROUTER, [takerToken, makerToken], takerFillAmounts),
                    ];
                    intermediateTokens.forEach(t => {
                        ops.push(this.getUniswapV2SellQuotes(constants_1.UNISWAPV2_ROUTER, [takerToken, t, makerToken], takerFillAmounts));
                    });
                    return ops;
                case types_1.ERC20BridgeSource.SushiSwap:
                    if (!bridge_source_utils_1.isValidAddress(constants_1.SUSHISWAP_ROUTER)) {
                        return [];
                    }
                    const sushiOps = [
                        this.getUniswapV2SellQuotes(constants_1.SUSHISWAP_ROUTER, [takerToken, makerToken], takerFillAmounts, types_1.ERC20BridgeSource.SushiSwap),
                    ];
                    intermediateTokens.forEach(t => {
                        sushiOps.push(this.getUniswapV2SellQuotes(constants_1.SUSHISWAP_ROUTER, [takerToken, t, makerToken], takerFillAmounts, types_1.ERC20BridgeSource.SushiSwap));
                    });
                    return sushiOps;
                case types_1.ERC20BridgeSource.CryptoCom:
                    if (!bridge_source_utils_1.isValidAddress(constants_1.CRYPTO_COM_ROUTER)) {
                        return [];
                    }
                    const cryptoComOps = [
                        this.getUniswapV2SellQuotes(constants_1.CRYPTO_COM_ROUTER, [takerToken, makerToken], takerFillAmounts, types_1.ERC20BridgeSource.CryptoCom),
                    ];
                    intermediateTokens.forEach(t => {
                        cryptoComOps.push(this.getUniswapV2SellQuotes(constants_1.CRYPTO_COM_ROUTER, [takerToken, t, makerToken], takerFillAmounts, types_1.ERC20BridgeSource.CryptoCom));
                    });
                    return cryptoComOps;
                ...
                case types_1.ERC20BridgeSource.MStable:
                    ...
                    return bridge_source_utils_1.isValidAddress(constants_1.MSTABLE_ROUTER)
                        ? this.getMStableSellQuotes(constants_1.MSTABLE_ROUTER, makerToken, takerToken, takerFillAmounts)
                        : [];
                case types_1.ERC20BridgeSource.Mooniswap:
                    return constants_1.MOONISWAP_REGISTRIES
                        ? [
                            ...constants_1.MOONISWAP_REGISTRIES.filter(r => bridge_source_utils_1.isValidAddress(r)).map(registry => this.getMooniswapSellQuotes(registry, makerToken, takerToken, takerFillAmounts)),
                        ]
                        : [];
                ...
                case types_1.ERC20BridgeSource.Shell:
                    return bridge_source_utils_1.getShellsForPair(takerToken, makerToken).map(pool => this.getShellSellQuotes(pool, makerToken, takerToken, takerFillAmounts));
                ...
                case types_1.ERC20BridgeSource.DodoV2:
                    return _.flatten(constants_1.DODOV2_FACTORIES.filter(factory => bridge_source_utils_1.isValidAddress(factory)).map(factory => bridge_source_utils_1.getDodoV2Offsets().map(offset => this.getDODOV2SellQuotes(factory, offset, makerToken, takerToken, takerFillAmounts))));
                ...
                case types_1.ERC20BridgeSource.Linkswap:
                    if (!bridge_source_utils_1.isValidAddress(constants_1.LINKSWAP_ROUTER)) {
                        return [];
                    }
                    const linkOps = [
                        this.getUniswapV2SellQuotes(constants_1.LINKSWAP_ROUTER, [takerToken, makerToken], takerFillAmounts, types_1.ERC20BridgeSource.Linkswap),
                    ];
                    ...
                    multihop_utils_1.getIntermediateTokens(makerToken, takerToken, {
                        ...
                        linkOps.push(this.getUniswapV2SellQuotes(constants_1.LINKSWAP_ROUTER, [takerToken, t, makerToken], takerFillAmounts, types_1.ERC20BridgeSource.Linkswap));
                    });
                    return linkOps;
                case types_1.ERC20BridgeSource.PancakeSwap:
                case types_1.ERC20BridgeSource.BakerySwap:
                    const router = source === types_1.ERC20BridgeSource.PancakeSwap ? constants_1.PANCAKESWAP_ROUTER : constants_1.BAKERYSWAP_ROUTER;
                    if (!bridge_source_utils_1.isValidAddress(router)) {
                        return [];
                    }
                    const pancakeOps = [
                        this.getUniswapV2SellQuotes(router, [takerToken, makerToken], takerFillAmounts, source),
                    ];
                    intermediateTokens.forEach(t => {
                        pancakeOps.push(this.getUniswapV2SellQuotes(router, [takerToken, t, makerToken], takerFillAmounts, source));
                    });
                    return pancakeOps;
                default:
        ...
        return _.flatten(_sources.map((source) => {
            switch (source) {
                case types_1.ERC20BridgeSource.Eth2Dai:
                    ...
                    return bridge_source_utils_1.isValidAddress(constants_1.OASIS_ROUTER)
                        ? this.getEth2DaiBuyQuotes(constants_1.OASIS_ROUTER, makerToken, takerToken, makerFillAmounts)
                        : [];
                case types_1.ERC20BridgeSource.Uniswap:
                    return bridge_source_utils_1.isValidAddress(constants_1.UNISWAPV1_ROUTER)
                        ? this.getUniswapBuyQuotes(constants_1.UNISWAPV1_ROUTER, makerToken, takerToken, makerFillAmounts)
                        : [];
                case types_1.ERC20BridgeSource.UniswapV2:
                    if (!bridge_source_utils_1.isValidAddress(constants_1.UNISWAPV2_ROUTER)) {
                        return [];
                    ...
                    const ops = [
                        this.getUniswapV2BuyQuotes(constants_1.UNISWAPV2_ROUTER, [takerToken, makerToken], makerFillAmounts),
                    ];
                    intermediateTokens.forEach(t => {
                        ops.push(this.getUniswapV2BuyQuotes(constants_1.UNISWAPV2_ROUTER, [takerToken, t, makerToken], makerFillAmounts));
                    });
                    return ops;
                case types_1.ERC20BridgeSource.SushiSwap:
                    if (!bridge_source_utils_1.isValidAddress(constants_1.SUSHISWAP_ROUTER)) {
                    ...
                    const sushiOps = [
                        this.getUniswapV2BuyQuotes(constants_1.SUSHISWAP_ROUTER, [takerToken, makerToken], makerFillAmounts, types_1.ERC20BridgeSource.SushiSwap),
                    ];
                    intermediateTokens.forEach(t => {
                        sushiOps.push(this.getUniswapV2BuyQuotes(constants_1.SUSHISWAP_ROUTER, [takerToken, t, makerToken], makerFillAmounts, types_1.ERC20BridgeSource.SushiSwap));
                    });
                    return sushiOps;
                case types_1.ERC20BridgeSource.CryptoCom:
                    if (!bridge_source_utils_1.isValidAddress(constants_1.CRYPTO_COM_ROUTER)) {
                    ...
                    const cryptoComOps = [
                        this.getUniswapV2BuyQuotes(constants_1.CRYPTO_COM_ROUTER, [takerToken, makerToken], makerFillAmounts, types_1.ERC20BridgeSource.CryptoCom),
                    ];
                    intermediateTokens.forEach(t => {
                        cryptoComOps.push(this.getUniswapV2BuyQuotes(constants_1.CRYPTO_COM_ROUTER, [takerToken, t, makerToken], makerFillAmounts, types_1.ERC20BridgeSource.CryptoCom));
                    });
                    return cryptoComOps;
                case types_1.ERC20BridgeSource.Kyber:
                ...
                case types_1.ERC20BridgeSource.MStable:
                    return bridge_source_utils_1.isValidAddress(constants_1.MSTABLE_ROUTER)
                        ? this.getMStableBuyQuotes(constants_1.MSTABLE_ROUTER, makerToken, takerToken, makerFillAmounts)
                        : [];
                case types_1.ERC20BridgeSource.Mooniswap:
                    if (!constants_1.MOONISWAP_REGISTRIES) {
                        return [];
                    }
                    return [
                        ...constants_1.MOONISWAP_REGISTRIES.filter(r => bridge_source_utils_1.isValidAddress(r)).map(registry => this.getMooniswapBuyQuotes(registry, makerToken, takerToken, makerFillAmounts)),
                    ];
                case types_1.ERC20BridgeSource.Balancer:
                    return this.balancerPoolsCache
                        .getCachedPoolAddressesForPair(takerToken, makerToken)
                ...
                case types_1.ERC20BridgeSource.Shell:
                    return bridge_source_utils_1.getShellsForPair(takerToken, makerToken).map(pool => this.getShellBuyQuotes(pool, makerToken, takerToken, makerFillAmounts));
                case types_1.ERC20BridgeSource.Dodo:
                    return this.getDODOBuyQuotes(makerToken, takerToken, makerFillAmounts);
                case types_1.ERC20BridgeSource.DodoV2:
                    return _.flatten(constants_1.DODOV2_FACTORIES.filter(factory => bridge_source_utils_1.isValidAddress(factory)).map(factory => bridge_source_utils_1.getDodoV2Offsets().map(offset => this.getDODOV2BuyQuotes(factory, offset, makerToken, takerToken, makerFillAmounts))));
                case types_1.ERC20BridgeSource.Bancor:
                ...
                case types_1.ERC20BridgeSource.Linkswap:
                    ...
                    const linkOps = [
                        this.getUniswapV2BuyQuotes(constants_1.LINKSWAP_ROUTER, [takerToken, makerToken], makerFillAmounts, types_1.ERC20BridgeSource.Linkswap),
                    ];
                    // LINK is the base asset in many of the pools on Linkswap
                    multihop_utils_1.getIntermediateTokens(makerToken, takerToken, {
                        default: [constants_1.TOKENS.LINK, constants_1.TOKENS.WETH],
                    }).forEach(t => {
                        linkOps.push(this.getUniswapV2BuyQuotes(constants_1.LINKSWAP_ROUTER, [takerToken, t, makerToken], makerFillAmounts, types_1.ERC20BridgeSource.Linkswap));
                    });
                    return linkOps;
                default:
...
//# sourceMappingURL=sampler_operations_BASE_37624.js.map