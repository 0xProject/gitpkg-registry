...
function getBestTwoHopQuote(marketSideLiquidity, feeSchedule) {
    ...
    const best = twoHopQuotes
        .map(quote => rate_utils_1.getTwoHopAdjustedRate(side, quote, inputAmount, ethToOutputRate, feeSchedule))
        .reduce((prev, curr, i) => curr.isGreaterThan(prev.adjustedRate) ? { adjustedRate: curr, quote: twoHopQuotes[i] } : prev, {
        adjustedRate: rate_utils_1.getTwoHopAdjustedRate(side, twoHopQuotes[0], inputAmount, ethToOutputRate, feeSchedule),
