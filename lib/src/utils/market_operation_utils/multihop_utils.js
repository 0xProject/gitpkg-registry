 ...
 */
function getBestTwoHopQuote(marketSideLiquidity, gasPrice, exchangeProxyOverhead) {
    ...
    const best = filteredQuotes
        .map(quote => rate_utils_1.getTwoHopAdjustedRate(side, quote, inputAmount, outputAmountPerEth, gasPrice, exchangeProxyOverhead))
        .reduce((prev, curr, i) => curr.isGreaterThan(prev.adjustedRate) ? { adjustedRate: curr, quote: filteredQuotes[i] } : prev, {
        adjustedRate: rate_utils_1.getTwoHopAdjustedRate(side, filteredQuotes[0], inputAmount, outputAmountPerEth, gasPrice, exchangeProxyOverhead),
