...
const interpolateOutputFromSamples = (left, right, targetInput) => left.output.plus(right.output
    .minus(left.output)
    .dividedBy(right.input.minus(left.input))
    .times(targetInput.minus(left.input)));
function findRoutesAndCreateOptimalPath(side, samples, nativeOrders, input, opts, fees) {
    ...
    const allSourcesRustRoute = neon_router_1.route(rustArgs, RUST_ROUTER_NUM_SAMPLES);
    ...
    for (const [routeInput, routeSamplesAndNativeOrders] of routesAndSamples) {
        ...
        for (let k = routeSamples.length - 1; k >= 0; k--) {
            ...
            if (rustInputAdjusted.isGreaterThan(routeSamples[k].input)) {
                ...
                if (left && right) {
                    ...
                    const interpolatedOutput = interpolateOutputFromSamples(left, right, rustInputAdjusted).decimalPlaces(0, side === types_1.MarketOperation.Sell ? utils_1.BigNumber.ROUND_FLOOR : utils_1.BigNumber.ROUND_CEIL);
                    fill = createFill(Object.assign(Object.assign({}, right), { input: rustInputAdjusted, output: interpolatedOutput }));
                }
                else {
        ...
        adjustedFills.push(Object.assign(Object.assign({}, fill), { input: rustInputAdjusted, output: scaleOutput(fill.output), adjustedOutput: scaleOutput(fill.adjustedOutput), index: 0, parent: undefined }));
    }
...
function findOptimalRustPathFromSamples(side, samples, nativeOrders, input, opts, fees, chainId) {
    ...
    if (vipSources.length > 0) {
        ...
        if (vipSourcesSamples.length > 0) {
            ...
            if (vipSourcesPath === null || vipSourcesPath === void 0 ? void 0 : vipSourcesPath.adjustedRate().isGreaterThan(allSourcesAdjustedRateWithFqtOverhead)) {
                logPerformance();
