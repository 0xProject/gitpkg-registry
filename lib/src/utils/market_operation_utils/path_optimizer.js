...
function calculateOuputFee(side, sampleOrNativeOrder, outputAmountPerEth, inputAmountPerEth, gasPrice) {
    if (isDexSample(sampleOrNativeOrder)) {
        ...
        const outputFee = fills_1.ethToOutputAmount({
            ...
            inputAmountPerEth,
            outputAmountPerEth,
            ethAmount: gasPrice.times(sampleOrNativeOrder.gasCost),
        });
        return outputFee;
    }
}
function findRoutesAndCreateOptimalPath(side, samples, nativeOrders, input, opts, neonRouterNumSamples, vipSourcesSet) {
    var _a;
    ...
    const createFill = (sample) => {
        const fills = fills_1.dexSamplesToFills(side, [sample], opts.outputAmountPerEth, opts.inputAmountPerEth, opts.gasPrice);
    ...
    const createPathFromStrategy = (sourcesRustRoute, sourcesOutputAmounts) => {
        ...
        for (const [routeInput, routeSamplesAndNativeOrders, outputAmount, sourcePathId,] of routesAndSamplesAndOutputs) {
            ...
            if (!isDexSample(current)) {
                const nativeFill = fills_1.nativeOrdersToFills(side, [current], rustInputAdjusted, opts.outputAmountPerEth, opts.inputAmountPerEth, opts.gasPrice)[0];
    ...
    for (const singleSourceSamples of samples) {
        ...
        const serializedPath = singleSourceSamplesWithOutput.reduce((memo, sample, sampleIdx) => {
            ...
            memo.outputFees.push(calculateOuputFee(side, sample, opts.outputAmountPerEth, opts.inputAmountPerEth, opts.gasPrice)
                .integerValue()
                .toNumber());
            return memo;
        }, {
...
function findOptimalRustPathFromSamples(side, samples, nativeOrders, input, opts, chainId, neonRouterNumSamples, samplerMetrics) {
    ...
    const paths = findRoutesAndCreateOptimalPath(side, samples, nativeOrders, input, opts, neonRouterNumSamples, vipSourcesSet);
    if (!paths) {
        sendMetrics();
        return undefined;
    }
