...
function findRoutesAndCreateOptimalPath(side, samples, nativeOrders, input, opts, fees, neonRouterNumSamples) {
    ...
    for (const [routeInput, routeSamplesAndNativeOrders, outputAmount] of routesAndSamplesAndOutputs) {
        ...
        const scaleOutput = (fillInput, output) => output
            .dividedBy(fillInput)
        ...
        adjustedFills.push(Object.assign(Object.assign({}, fill), { input: rustInputAdjusted, output: scaleOutput(fill.input, fill.output), adjustedOutput: scaleOutput(fill.input, fill.adjustedOutput), index: 0, parent: undefined }));
    }
