...
function calculateOuputFee(side, sampleOrNativeOrder, outputAmountPerEth, inputAmountPerEth, fees) {
    if (isDexSample(sampleOrNativeOrder)) {
        ...
        const fee = fees[types_2.ERC20BridgeSource.Native] === undefined ? 0 : fees[types_2.ERC20BridgeSource.Native](sampleOrNativeOrder);
        const outputFee = !outputAmountPerEth.isZero()
            ? outputAmountPerEth.times(fee)
...
function createPathFromSamples(side, samples, nativeOrders, input, opts, fees) {
    const createFill = (sampleOrOrder) => isDexSample(sampleOrOrder)
        ? fills_1.dexSamplesToFills(side, [sampleOrOrder], opts.outputAmountPerEth, opts.inputAmountPerEth, fees)[0]
        : fills_1.nativeOrdersToFills(side, [sampleOrOrder], input, opts.outputAmountPerEth, opts.inputAmountPerEth, fees)[0];
    // Track sample id's to integers (required by rust router)
    const sampleIdLookup = {};
    let sampleIdCounter = 0;
    const sampleToId = (source, index) => {
    ...
    for (const singleSourceSamples of samples) {
        const singleSourceSamplesWithOutput = singleSourceSamples.filter(sample => sample.output.isGreaterThan(constants_2.ZERO_AMOUNT));
    ...
    for (const [idx, nativeOrder] of nativeOrders.entries()) {
        ...
        const id = sampleToId(types_2.ERC20BridgeSource.Native, idx);
        const ids = [id, id, id];
    ...
    let allSourcesRustRoute = [];
    if (rustArgs.pathsIn.length > 0) {
        const before = perf_hooks_1.performance.now();
        allSourcesRustRoute = neon_router_1.route(rustArgs, RUST_ROUTER_NUM_SAMPLES);
        constants_1.DEFAULT_INFO_LOGGER({ router: 'neon-router', performanceMs: perf_hooks_1.performance.now() - before, type: 'real' }, 'Rust router real routing performance');
    }
    const routesAndSamples = _.zip(allSourcesRustRoute, samplesAndNativeOrdersWithResults);
    const adjustedFills = [];
    const totalInputs = utils_1.BigNumber.sum(...allSourcesRustRoute);
    for (const [routeInput, routeSamplesAndNativeOrders] of routesAndSamples) {
        ...
        const rustInput = new utils_1.BigNumber(routeInput);
        ...
        for (let k = routeSamples.length - 1; k >= 0; k--) {
            ...
            if (rustInput.isGreaterThan(routeSamples[k].input)) {
                ...
                if (left && right) {
                    ...
                    fill = createFill(Object.assign(Object.assign({}, right), { input: rustInput, output }));
                }
                else {
        ...
        const adjustedInput = totalInputs.lt(input) && adjustedFills.length === 0 ? rustInput.plus(input.minus(totalInputs)) : rustInput;
        const scaleOutput = (output) => output
            .dividedBy(fill.input)
            .times(adjustedInput)
            .decimalPlaces(0, side === types_1.MarketOperation.Sell ? utils_1.BigNumber.ROUND_FLOOR : utils_1.BigNumber.ROUND_CEIL);
        adjustedFills.push(Object.assign(Object.assign({}, fill), { input: adjustedInput, output: scaleOutput(fill.output), adjustedOutput: scaleOutput(fill.adjustedOutput), index: 0, parent: undefined }));
    }
    const pathFromRustInputs = path_1.Path.create(side, adjustedFills, input);
    return pathFromRustInputs;
}
function findOptimalRustPathFromSamples(side, samples, nativeOrders, input, opts, fees, chainId) {
    const before = perf_hooks_1.performance.now();
    const logPerformance = () => constants_1.DEFAULT_INFO_LOGGER({ router: 'neon-router', performanceMs: perf_hooks_1.performance.now() - before, type: 'total' }, 'Rust router total routing performance');
    const allSourcesPath = createPathFromSamples(side, samples, nativeOrders, input, opts, fees);
    const vipSources = constants_2.VIP_ERC20_BRIDGE_SOURCES_BY_CHAIN_ID[chainId];
    ...
    if (vipSources.length > 0) {
        ...
        if (vipSourcesSamples.length > 0) {
            ...
            const outputWithFqtOverhead = side === types_1.MarketOperation.Sell
                ? allSourcesOutput.minus(fqtOverheadInOutputToken)
                : allSourcesOutput.plus(fqtOverheadInOutputToken);
            const allSourcesAdjustedRateWithFqtOverhead = rate_utils_1.getRate(side, allSourcesInput, outputWithFqtOverhead);
 ...
 */
function findOptimalPathJSAsync(side, fills, targetInput, runLimit = Math.pow(2, 8), opts = path_1.DEFAULT_PATH_PENALTY_OPTS) {
