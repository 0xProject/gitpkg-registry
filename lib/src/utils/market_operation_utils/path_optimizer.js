...
function createPathFromSamples(side, samples, nativeOrders, input, opts, fees) {
    const createFill = (sample) => fills_1.dexSamplesToFills(side, [sample], opts.outputAmountPerEth, opts.inputAmountPerEth, fees)[0];
    ...
    for (const [routeInput, routeSamplesAndNativeOrders] of routesAndSamples) {
        ...
        const rustInput = new utils_1.BigNumber(routeInput).multipliedBy(scale).integerValue(utils_1.BigNumber.ROUND_CEIL);
        const current = routeSamplesAndNativeOrders[routeSamplesAndNativeOrders.length - 1];
        if (!isDexSample(current)) {
            const nativeFill = fills_1.nativeOrdersToFills(side, [current], rustInput, opts.outputAmountPerEth, opts.inputAmountPerEth, fees)[0];
        ...
        const adjustedInput = totalRoutedAmount.lt(input) && adjustedFills.length === 0
