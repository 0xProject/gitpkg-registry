...
function findRoutesAndCreateOptimalPath(side, samples, nativeOrders, input, opts, fees, neonRouterNumSamples, vipSourcesSet) {
    ...
    const createFill = (sample) => {
        const fills = (0, fills_1.dexSamplesToFills)(side, [sample], opts.outputAmountPerEth, opts.inputAmountPerEth, fees);
    ...
    const createPathFromStrategy = (sourcesRustRoute, sourcesOutputAmounts) => {
        ...
        for (const [routeInput, routeSamplesAndNativeOrders, outputAmount, sourcePathId,] of routesAndSamplesAndOutputs) {
            ...
            if (!isDexSample(current)) {
                const nativeFill = (0, fills_1.nativeOrdersToFills)(side, [current], rustInputAdjusted, opts.outputAmountPerEth, opts.inputAmountPerEth, fees, false)[0];
