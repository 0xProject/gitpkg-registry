...
function findRoutesAndCreateOptimalPath(side, samples, nativeOrders, input, opts, neonRouterNumSamples, gasPrice) {
    var _a, _b, _c;
    ...
    const createFill = (sample) => {
        const fills = fills_1.dexSamplesToFills(side, [sample], opts.outputAmountPerEth, opts.inputAmountPerEth, gasPrice);
    ...
    for (const singleSourceSamples of samples) {
        ...
        const serializedPath = singleSourceSamplesWithOutput.reduce((memo, sample, sampleIdx) => {
            ...
            memo.outputFees.push(calculateOuputFee(side, sample, opts.outputAmountPerEth, opts.inputAmountPerEth, gasPrice)
                .integerValue()
                .toNumber());
            return memo;
        }, {
    ...
    for (const [routeInput, routeSamplesAndNativeOrders, outputAmount, sourcePathId] of routesAndSamplesAndOutputs) {
        ...
        if (!isDexSample(current)) {
            const nativeFill = fills_1.nativeOrdersToFills(side, [current], rustInputAdjusted, opts.outputAmountPerEth, opts.inputAmountPerEth, gasPrice, false)[0];
        ...
        const scaleOutput = (output) => {
            ...
            return utils_1.BigNumber.max(utils_1.BigNumber.min(scaled, maxSampledOutput), 1);
        };
...
function findOptimalRustPathFromSamples(side, samples, nativeOrders, input, opts, gasPrice, chainId, neonRouterNumSamples, samplerMetrics) {
    const beforeAllTimeMs = perf_hooks_1.performance.now();
    let beforeTimeMs = perf_hooks_1.performance.now();
    const allSourcesPath = findRoutesAndCreateOptimalPath(side, samples, nativeOrders, input, opts, neonRouterNumSamples, gasPrice);
    ...
    if (vipSources.length > 0) {
        ...
        if (vipSourcesSamples.length > 0) {
            const vipSourcesPath = findRoutesAndCreateOptimalPath(side, vipSourcesSamples, [], input, opts, neonRouterNumSamples, gasPrice);
            // tslint:disable-next-line: no-unused-expression
            samplerMetrics &&
                samplerMetrics.logRouterDetails({
                    router: 'neon-router',
...
function findOptimalPathJSAsync(side, fills, targetInput, runLimit = Math.pow(2, 8), samplerMetrics, opts = path_1.DEFAULT_PATH_PENALTY_OPTS) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        console.log(finalPath === null || finalPath === void 0 ? void 0 : finalPath.fills.map(f => f.source), finalPath === null || finalPath === void 0 ? void 0 : finalPath.adjustedCompleteMakerToTakerRate());
        // tslint:disable-next-line: no-unused-expression
        samplerMetrics &&
            samplerMetrics.logRouterDetails({
                router: 'js',
