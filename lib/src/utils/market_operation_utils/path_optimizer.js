...
function findRoutesAndCreateOptimalPath(side, samples, nativeOrders, input, opts, fees, neonRouterNumSamples) {
    ...
    for (const [routeInput, routeSamplesAndNativeOrders, outputAmount, sourcePathId] of routesAndSamplesAndOutputs) {
        ...
        const scaleOutput = (output) => {
            ...
            return utils_1.BigNumber.max(utils_1.BigNumber.min(output.times(scale), maxSampledOutput), 1).decimalPlaces(0, side === types_1.MarketOperation.Sell ? utils_1.BigNumber.ROUND_FLOOR : utils_1.BigNumber.ROUND_CEIL);
        };
