...
function reducePaths(sortedPaths, side) {
    ...
    const filteredPaths = sortedPaths.filter(p => p.bestRate().isGreaterThanOrEqualTo(bestNonNativeCompletePath.adjustedCompleteRate()));
...
function rateBySourcePathId(_side, paths, _targetInput) {
