...
function findRoutesAndCreateOptimalPath(side, samples, nativeOrders, input, opts, fees, neonRouterNumSamples) {
    ...
    for (const [idx, nativeOrder] of nativeOrders.entries()) {
        ...
        const id = `${types_2.ERC20BridgeSource.Native}-${serializedPaths.length}-${idx}`;
        const ids = [id, id, id];
    ...
    const routesAndSamplesAndOutputs = _.zip(allSourcesRustRoute, samplesAndNativeOrdersWithResults, strategySourcesOutputAmounts);
    const adjustedFills = [];
    ...
    for (const [routeInput, routeSamplesAndNativeOrders, outputAmount] of routesAndSamplesAndOutputs) {
        ...
        for (let k = routeSamples.length - 1; k >= 0; k--) {
            ...
            if (rustInputAdjusted.isGreaterThan(routeSamples[k].input)) {
                ...
                if (left && right) {
                    fill = createFill(Object.assign(Object.assign({}, right), { input: rustInputAdjusted, output: new utils_1.BigNumber(outputAmount) }));
                }
                else {
...
function findOptimalRustPathFromSamples(side, samples, nativeOrders, input, opts, fees, chainId, neonRouterNumSamples) {
    const before = perf_hooks_1.performance.now();
