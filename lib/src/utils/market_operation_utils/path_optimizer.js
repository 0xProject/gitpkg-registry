...
function findRoutesAndCreateOptimalPath(side, samples, nativeOrders, input, opts, fees, neonRouterNumSamples, vipSourcesSet, chainId) {
    var _a;
    ...
    if (SHOULD_ENABLE_RUST_DATA_SAMPLER && Math.random() < router_test_data_sampler_1.SAMPLE_THRESHOLD) {
        router_test_data_sampler_1.TestDataSampler.sampleRoute(chainId, rustArgs);
    }
...
function findOptimalRustPathFromSamples(side, samples, nativeOrders, input, opts, fees, chainId, neonRouterNumSamples, samplerMetrics) {
    ...
    const paths = findRoutesAndCreateOptimalPath(side, samples, nativeOrders, input, opts, fees, neonRouterNumSamples, vipSourcesSet, chainId);
    if (!paths) {
        sendMetrics();
        return undefined;
    }
