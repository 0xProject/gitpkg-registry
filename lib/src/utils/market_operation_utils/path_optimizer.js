            ...
            switch (_e.label) {
                case 0:
                    sortedPaths = paths
                        .slice(0)
                        .sort(function (a, b) {
                        return fills_1.getPathAdjustedRate(side, b, targetInput).comparedTo(fills_1.getPathAdjustedRate(side, a, targetInput));
                    });
...
function mixPaths(side, pathA, pathB, targetInput, maxSteps) {
    var _maxSteps = Math.max(maxSteps, 16);
    var bestPath = pathA;
    var bestPathInput = fills_1.getPathSize(pathA, targetInput)[0];
    var bestPathRate = fills_1.getPathAdjustedRate(side, pathA, targetInput);
    var steps = 0;
    ...
    var _walk = function (path, input, output, allFills) {
        steps += 1;
        var rate = getRate(side, targetInput, output);
        ...
        if (remainingInput.gt(0)) {
            for (var i = 0; i < allFills.length && steps < _maxSteps; ++i) {
                var fill = allFills[i];
                var nextPath = __spread(path, [fill]);
                // Only walk valid paths.
                if (!fills_1.isValidPath(nextPath, true)) {
                    continue;
                }
                ...
                // Recurse.
                _walk(nextPath, input.plus(utils_1.BigNumber.min(remainingInput, fill.input)), output.plus(
    ...
    var allPaths = __spread(pathA, pathB);
    var sources = allPaths.filter(function (f) { return f.index === 0; }).map(function (f) { return f.sourcePathId; });
    var rateBySource = Object.assign.apply(Object, __spread([{}], sources.map(function (s) {
        var _a;
        return (_a = {},
            _a[s] = fills_1.getPathAdjustedRate(side, allPaths.filter(function (f) { return f.sourcePathId === s; }), targetInput),
            _a);
    })));
    _walk([], constants_1.ZERO_AMOUNT, constants_1.ZERO_AMOUNT, 
