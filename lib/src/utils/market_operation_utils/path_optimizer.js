...
const router_test_data_sampler_1 = require("./router_test_data_sampler");
...
function findRoutesAndCreateOptimalPath(side, inputToken, outputToken, samples, nativeOrders, input, opts, fees, neonRouterNumSamples, vipSourcesSet, chainId) {
    var _a;
    ...
    const createPathFromStrategy = (sourcesRustRoute, sourcesOutputAmounts) => {
        var _a, _b, _c;
        const routesAndSamplesAndOutputs = _.zip(sourcesRustRoute, samplesAndNativeOrdersWithResults, sourcesOutputAmounts, sampleSourcePathIds);
        const adjustedFills = [];
        const totalRoutedAmount = utils_1.BigNumber.sum(...sourcesRustRoute);
        ...
        for (const [routeInput, routeSamplesAndNativeOrders, outputAmount, sourcePathId,] of routesAndSamplesAndOutputs) {
            if (!Number.isFinite(outputAmount)) {
                constants_1.DEFAULT_WARNING_LOGGER(rustArgs, `neon-router: invalid route outputAmount ${outputAmount}`);
                return undefined;
            }
            ...
            if (!isDexSample(current)) {
                const nativeFill = fills_1.nativeOrdersToFills(side, [current], rustInputAdjusted, opts.outputAmountPerEth, opts.inputAmountPerEth, fees, false)[0];
            ...
            const scaleOutput = (output) => {
                ...
                if (output.eq(constants_2.ZERO_AMOUNT)) {
                    return output;
                }
                const scaled = output
                    .times(scale)
                    .decimalPlaces(0, side === types_1.MarketOperation.Sell ? utils_1.BigNumber.ROUND_FLOOR : utils_1.BigNumber.ROUND_CEIL);
                const capped = types_1.MarketOperation.Sell ? utils_1.BigNumber.min(scaled, maxSampledOutput) : scaled;
    ...
    if (SHOULD_ENABLE_RUST_DATA_SAMPLER && Math.random() < router_test_data_sampler_1.SAMPLE_THRESHOLD) {
        router_test_data_sampler_1.TestDataSampler.sampleRoute(chainId, Object.assign(Object.assign({}, rustArgs), { inputToken, outputToken }));
    }
    neon_router_1.route(rustArgs, allSourcesRustRoute, allSourcesOutputAmounts, vipSourcesRustRoute, vipSourcesOutputAmounts, neonRouterNumSamples);
...
function findOptimalRustPathFromSamples(side, inputToken, outputToken, samples, nativeOrders, input, opts, fees, chainId, neonRouterNumSamples, samplerMetrics) {
    ...
    const paths = findRoutesAndCreateOptimalPath(side, inputToken, outputToken, samples, nativeOrders, input, opts, fees, neonRouterNumSamples, vipSourcesSet, chainId);
    if (!paths) {
        sendMetrics();
        return undefined;
    }
    const { allSourcesPath, vipSourcesPath } = paths;
    if (!allSourcesPath || (vipSourcesPath === null || vipSourcesPath === void 0 ? void 0 : vipSourcesPath.isBetterThan(allSourcesPath))) {
        sendMetrics();
