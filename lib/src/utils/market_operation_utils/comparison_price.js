...
function getComparisonPrices(adjustedRate, amount, marketSideLiquidity, feeSchedule, exchangeProxyOverhead) {
    ...
    if (feeSchedule[types_2.ERC20BridgeSource.Native] === undefined) {
        utils_1.logUtils.warn('ComparisonPrice function did not find native order fee schedule');
        return { wholeOrder };
    }
    else {
        try {
            const fillFeeInEth = new utils_1.BigNumber(feeSchedule[types_2.ERC20BridgeSource.Native]({ type: protocol_utils_1.FillQuoteTransformerOrderType.Rfq }));
            const exchangeProxyOverheadInEth = new utils_1.BigNumber(exchangeProxyOverhead(constants_1.SOURCE_FLAGS.Native));
