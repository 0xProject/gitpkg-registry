 ...
 */
function getComparisonPrices(adjustedRate, amount, marketSideLiquidity, feeSchedule, exchangeProxyOverhead) {
    let wholeOrder;
    let feeInEth;
    ...
    if (feeSchedule[types_1.ERC20BridgeSource.Native] === undefined) {
        utils_1.logUtils.warn('ComparisonPrice function did not find native order fee schedule');
        return { wholeOrder };
    }
    else {
        try {
            const fillFeeInEth = new utils_1.BigNumber(feeSchedule[types_1.ERC20BridgeSource.Native]({ type: protocol_utils_1.FillQuoteTransformerOrderType.Rfq }));
            const exchangeProxyOverheadInEth = new utils_1.BigNumber(exchangeProxyOverhead(constants_1.SOURCE_FLAGS.RfqOrder, 1));
    ...
    const feePenalty = !marketSideLiquidity.outputAmountPerEth.isZero()
        ...
        : // if it's a sell, the input token is the taker token
            marketSideLiquidity.inputAmountPerEth
                .times(feeInEth)
                .times(marketSideLiquidity.side === types_2.MarketOperation.Sell ? adjustedRate : adjustedRate.pow(-1));
    // the adjusted rate is defined as maker/taker
    // input is the taker token for sells, input is the maker token for buys
    const orderMakerAmount = marketSideLiquidity.side === types_2.MarketOperation.Sell ? adjustedRate.times(amount).plus(feePenalty) : amount;
    const orderTakerAmount = marketSideLiquidity.side === types_2.MarketOperation.Sell ? amount : amount.dividedBy(adjustedRate).minus(feePenalty);
