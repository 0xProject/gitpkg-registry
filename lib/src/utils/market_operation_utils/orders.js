...
function createBridgeOrder(fill, opts) {
    ...
    var bridgeAddress = getBridgeAddressFromFill(fill, opts);
    var makerAssetData;
    switch (fill.source) {
        case types_2.ERC20BridgeSource.Curve:
        ...
        case types_2.ERC20BridgeSource.Balancer:
            ...
            break;
        case types_2.ERC20BridgeSource.Bancor:
            var bancorFillData = fill.fillData; // tslint:disable-line:no-non-null-assertion
...
function createBancorBridgeData(path, bancorNetworkAddress) {
    var encoder = utils_1.AbiEncoder.create([
        { name: 'path', type: 'address[]' },
        { name: 'networkAddress', type: 'address' },
    ]);
    return encoder.encode({ path: path, bancorNetworkAddress: bancorNetworkAddress });
}
