...
const dexForwarderBridgeDataEncoder = utils_1.AbiEncoder.create([
...
function createOrdersFromPath(path, opts) {
    ...
    const collapsedPath = fills_1.collapsePath(path);
    const orders = [];
...
function createBridgeOrder(fill, makerToken, takerToken, opts) {
    ...
    switch (fill.source) {
        case types_2.ERC20BridgeSource.Curve:
            ...
            makerAssetData = order_utils_1.assetDataUtils.encodeERC20BridgeAssetData(makerToken, bridgeAddress, createCurveBridgeData(curveFillData.curve.poolAddress, curveFillData.curve.exchangeFunctionSelector, takerToken, curveFillData.fromTokenIdx, curveFillData.toTokenIdx));
            break;
        case types_2.ERC20BridgeSource.Swerve:
        ...
        case types_2.ERC20BridgeSource.Balancer:
            ...
            makerAssetData = order_utils_1.assetDataUtils.encodeERC20BridgeAssetData(makerToken, bridgeAddress, createBalancerBridgeData(takerToken, makerToken));
            break;
        case types_2.ERC20BridgeSource.Bancor:
...
function createBatchedBridgeOrder(fills, opts) {
    const [makerToken, takerToken] = getMakerTakerTokens(opts);
    let totalMakerAssetAmount = constants_1.ZERO_AMOUNT;
    let totalTakerAssetAmount = constants_1.ZERO_AMOUNT;
    const batchedBridgeData = {
    ...
    for (const fill of fills) {
        ...
        const { bridgeAddress, bridgeData: orderBridgeData } = order_utils_1.assetDataUtils.decodeAssetDataOrThrow(bridgeOrder.makerAssetData);
