...
function getBridgeAddressFromSource(source, opts) {
    switch (source) {
        ...
        case types_2.ERC20BridgeSource.UniswapV2Eth:
        ...
        case types_2.ERC20BridgeSource.CurveUsdcDaiUsdtSusd:
...
function createBridgeOrder(fill, opts) {
    ...
    switch (fill.source) {
        ...
        case types_2.ERC20BridgeSource.CurveUsdcDaiUsdtBusd:
        case types_2.ERC20BridgeSource.CurveUsdcDaiUsdtSusd:
            var _b = source_utils_1.getCurveInfo(fill.source, takerToken, makerToken), curveAddress = _b.curveAddress, fromTokenIdx = _b.fromTokenIdx, toTokenIdx = _b.toTokenIdx, version = _b.version;
            ...
            break;
        case types_2.ERC20BridgeSource.UniswapV2:
            makerAssetData = order_utils_1.assetDataUtils.encodeERC20BridgeAssetData(makerToken, bridgeAddress, createUniswapV2BridgeData([takerToken, makerToken]));
            break;
        case types_2.ERC20BridgeSource.UniswapV2Eth:
            if (opts.contractAddresses.etherToken === constants_1.NULL_ADDRESS) {
                throw new Error("Cannot create a " + types_2.ERC20BridgeSource.UniswapV2Eth.toString() + " order without a WETH address");
            }
            makerAssetData = order_utils_1.assetDataUtils.encodeERC20BridgeAssetData(makerToken, bridgeAddress, createUniswapV2BridgeData([takerToken, opts.contractAddresses.etherToken, makerToken]));
            break;
        case types_2.ERC20BridgeSource.MultiBridge:
    ...
    var _c = __read(getSlippedBridgeAssetAmounts(fill, opts), 2), slippedMakerAssetAmount = _c[0], slippedTakerAssetAmount = _c[1];
...
function createNativeOrder(fill) {
    return __assign({ fills: [fill] }, fill.nativeOrder);
}
