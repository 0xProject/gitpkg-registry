...
exports.DEFAULT_PATH_PENALTY_OPTS = {
    ethToOutputRate: constants_1.ZERO_AMOUNT,
    ethToInputRate: constants_1.ZERO_AMOUNT,
    ...
    collapse(opts) {
        ...
        for (let i = 0; i < collapsedFills.length;) {
            if (collapsedFills[i].source === types_2.ERC20BridgeSource.Native) {
                this.orders.push(orders_1.createNativeOrder(collapsedFills[i]));
                ++i;
                continue;
            }
            ...
            for (let j = i + 1; j < collapsedFills.length; ++j) {
                if (collapsedFills[j].source === types_2.ERC20BridgeSource.Native) {
                    break;
                }
            ...
            if (!opts.shouldBatchBridgeOrders) {
                this.orders.push(orders_1.createBridgeOrder(contiguousBridgeFills[0], makerToken, takerToken, opts));
                i += 1;
            }
            else {
                this.orders.push(orders_1.createBatchedBridgeOrder(contiguousBridgeFills, opts));
...
exports.Path = Path;
const MULTIBRIDGE_SOURCES = constants_1.SOURCE_FLAGS.LiquidityProvider | constants_1.SOURCE_FLAGS.Uniswap;
function doSourcesConflict(flags) {
    const multiBridgeConflict = flags & constants_1.SOURCE_FLAGS.MultiBridge && flags & MULTIBRIDGE_SOURCES;
    return !multiBridgeConflict;
}
exports.doSourcesConflict = doSourcesConflict;
