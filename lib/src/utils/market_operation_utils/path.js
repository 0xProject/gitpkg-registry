...
class Path {
    constructor(side, fills, targetInput, pathPenaltyOpts) {
        ...
        this._fallbackFillsStartIndex = 0;
    }
    ...
    collapse(opts) {
        ...
        for (let i = 0; i < collapsedFills.length; ++i) {
            ...
            this.orders.push(orders_1.createBridgeOrder(collapsedFills[i], opts.inputToken, opts.outputToken));
        }
        return this;
    }
    size() {
    ...
    completeSize() {
        ...
        if (this.targetInput.eq(0) || input.eq(0) || output.eq(0)) {
            return { input: constants_1.ZERO_AMOUNT, output: constants_1.ZERO_AMOUNT };
        }
    ...
    adjustedSize() {
        ...
        const pathPenalty = fills_1.ethToOutputAmount({
            input,
            output,
            inputAmountPerEth,
            outputAmountPerEth,
    ...
    adjustedCompleteTakerToMakerRate() {
        const { input, output } = this.adjustedSize();
        return rate_utils_1.getCompleteTakerToMakerRate(this.side, input, output, this.targetInput);
    }
    adjustedRate() {
        const { input, output } = this.adjustedSize();
