 ...
 */
function getTwoHopAdjustedRate(side, twoHopQuote, targetInput, ethToOutputRate, fees = {}, exchangeProxyOverhead = () => constants_1.ZERO_AMOUNT) {
    const { output, input, fillData } = twoHopQuote;
    if (input.isLessThan(targetInput) || output.isZero()) {
        return constants_1.ZERO_AMOUNT;
    }
    const penalty = ethToOutputRate.times(exchangeProxyOverhead(constants_1.SOURCE_FLAGS.MultiHop).plus(fees[types_2.ERC20BridgeSource.MultiHop](fillData)));
