...
class SamplerOperations {
    constructor(chainId, _samplerContract, balancerPoolsCache = new balancer_utils_1.BalancerPoolsCache(), creamPoolsCache = new cream_utils_1.CreamPoolsCache(), tokenAdjacencyGraph = { default: [] }, liquidityProviderRegistry = constants_1.LIQUIDITY_PROVIDER_REGISTRY, bancorServiceFn = () => __awaiter(this, void 0, void 0, function* () { return undefined; })) {
    ...
    getKyberSellQuotes(kyberOpts, reserveOffset, makerToken, takerToken, takerFillAmounts) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            params: [Object.assign({}, kyberOpts, { reserveOffset, hint: constants_1.NULL_BYTES }), takerToken, makerToken, takerFillAmounts],
    ...
    getKyberBuyQuotes(kyberOpts, reserveOffset, makerToken, takerToken, makerFillAmounts) {
        return new sampler_contract_operation_1.SamplerContractOperation({
            ...
            params: [Object.assign({}, kyberOpts, { reserveOffset, hint: constants_1.NULL_BYTES }), takerToken, makerToken, makerFillAmounts],
            callback: (callResults, fillData) => {
        ...
        const allOps = _.flatten(_sources.map((source) => {
            switch (source) {
                ...
                case types_1.ERC20BridgeSource.Curve:
                    return bridge_source_utils_1.getCurveInfosForPair(this.chainId, takerToken, makerToken).map(pool => this.getCurveSellQuotes(pool, pool.tokens.indexOf(takerToken), pool.tokens.indexOf(makerToken), takerFillAmounts));
                case types_1.ERC20BridgeSource.Swerve:
                    return bridge_source_utils_1.getSwerveInfosForPair(this.chainId, takerToken, makerToken).map(pool => this.getSwerveSellQuotes(pool, pool.tokens.indexOf(takerToken), pool.tokens.indexOf(makerToken), takerFillAmounts));
                case types_1.ERC20BridgeSource.SnowSwap:
                    return bridge_source_utils_1.getSnowSwapInfosForPair(this.chainId, takerToken, makerToken).map(pool => this.getSnowSwapSellQuotes(pool, pool.tokens.indexOf(takerToken), pool.tokens.indexOf(makerToken), takerFillAmounts));
                ...
                case types_1.ERC20BridgeSource.MStable:
                    ...
                    return bridge_source_utils_1.isValidAddress(constants_1.MSTABLE_ROUTER_BY_CHAIN_ID[this.chainId])
                        ? this.getMStableSellQuotes(constants_1.MSTABLE_ROUTER_BY_CHAIN_ID[this.chainId], makerToken, takerToken, takerFillAmounts)
                        : [];
                ...
                case types_1.ERC20BridgeSource.Shell:
                    return bridge_source_utils_1.getShellsForPair(this.chainId, takerToken, makerToken).map(pool => this.getShellSellQuotes(pool, makerToken, takerToken, takerFillAmounts));
                case types_1.ERC20BridgeSource.Dodo:
                    if (!bridge_source_utils_1.isValidAddress(constants_1.DODO_CONFIG_BY_CHAIN_ID[this.chainId])) {
                        return [];
                    }
                    return this.getDODOSellQuotes(constants_1.DODO_CONFIG_BY_CHAIN_ID[this.chainId], makerToken, takerToken, takerFillAmounts);
                ...
                case types_1.ERC20BridgeSource.Linkswap:
                    ...
                    return [
                        [takerToken, makerToken],
                        ...[constants_1.TOKENS.LINK, constants_1.TOKENS.WETH].map(t => [takerToken, t, makerToken]),
    ...
    _getBuyQuoteOperations(sources, makerToken, takerToken, makerFillAmounts) {
        ...
        return _.flatten(_sources.map((source) => {
            switch (source) {
                ...
                case types_1.ERC20BridgeSource.Curve:
                    return bridge_source_utils_1.getCurveInfosForPair(this.chainId, takerToken, makerToken).map(pool => this.getCurveBuyQuotes(pool, pool.tokens.indexOf(takerToken), pool.tokens.indexOf(makerToken), makerFillAmounts));
                case types_1.ERC20BridgeSource.Swerve:
                    return bridge_source_utils_1.getSwerveInfosForPair(this.chainId, takerToken, makerToken).map(pool => this.getSwerveBuyQuotes(pool, pool.tokens.indexOf(takerToken), pool.tokens.indexOf(makerToken), makerFillAmounts));
                case types_1.ERC20BridgeSource.SnowSwap:
                    return bridge_source_utils_1.getSnowSwapInfosForPair(this.chainId, takerToken, makerToken).map(pool => this.getSnowSwapBuyQuotes(pool, pool.tokens.indexOf(takerToken), pool.tokens.indexOf(makerToken), makerFillAmounts));
                case types_1.ERC20BridgeSource.LiquidityProvider:
                ...
                case types_1.ERC20BridgeSource.MStable:
                    return bridge_source_utils_1.isValidAddress(constants_1.MSTABLE_ROUTER_BY_CHAIN_ID[this.chainId])
                        ? this.getMStableBuyQuotes(constants_1.MSTABLE_ROUTER_BY_CHAIN_ID[this.chainId], makerToken, takerToken, makerFillAmounts)
                        : [];
                case types_1.ERC20BridgeSource.Mooniswap:
                    return [
                        ...constants_1.MOONISWAP_REGISTRIES_BY_CHAIN_ID[this.chainId]
                ...
                case types_1.ERC20BridgeSource.Shell:
                    return bridge_source_utils_1.getShellsForPair(this.chainId, takerToken, makerToken).map(pool => this.getShellBuyQuotes(pool, makerToken, takerToken, makerFillAmounts));
                case types_1.ERC20BridgeSource.Dodo:
                    if (!bridge_source_utils_1.isValidAddress(constants_1.DODO_CONFIG_BY_CHAIN_ID[this.chainId])) {
                        return [];
                    }
                    return this.getDODOBuyQuotes(constants_1.DODO_CONFIG_BY_CHAIN_ID[this.chainId], makerToken, takerToken, makerFillAmounts);
                case types_1.ERC20BridgeSource.DodoV2:
                    return _.flatten(constants_1.DODOV2_FACTORIES_BY_CHAIN_ID[this.chainId]
                        .filter(factory => bridge_source_utils_1.isValidAddress(factory))
...
//# sourceMappingURL=sampler_operations_REMOTE_38151.js.map