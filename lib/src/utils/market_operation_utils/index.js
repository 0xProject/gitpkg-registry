...
class MarketOperationUtils {
    ...
    getPricesAsync(nativeOrders, takerAmount, opts) {
        return __awaiter(this, void 0, void 0, function* () {
            if (nativeOrders.length === 0) {
                throw new Error(types_2.AggregationError.EmptyOrders);
            }
            ...
            const quoteSourceFilters = this._sellSources.merge(requestFilters);
            const feeSourceFilters = this._feeSources.exclude(_opts.excludedFeeSources);
            const tokens = nativeOrders.map(o => orders_1.getNativeOrderTokens(o));
            const [ethToTakerRate, samplerResults, decimals] = yield this._sampler.executeAsync(this._sampler.getMedianSellRate(feeSourceFilters.sources, tokens[0][1], this._wethAddress, constants_1.ONE_ETHER), (() => {
                const ops = tokens.map(t => this._sampler.getSellQuotes(quoteSourceFilters.sources, t[0], t[1], [takerAmount]));
                return {
                    encodeCall: () => this._sampler._samplerContract
                        .batchCall(ops.map(o => o.encodeCall()))
                        .getABIEncodedTransactionData(),
                    handleCallResults: (callResults) => {
                        const rawSubCallResults = this._sampler._samplerContract.getABIDecodedReturnData('batchCall', callResults);
                        const samples = ops.map((op, i) => op.handleCallResults(rawSubCallResults[i]));
                        return samples;
                    },
                };
            })(), (() => {
                const ops = tokens.map(t => this._sampler.getTokenDecimals(t[0], t[1]));
                return {
                    encodeCall: () => this._sampler._samplerContract
                        .batchCall(ops.map(o => o.encodeCall()))
                        .getABIEncodedTransactionData(),
                    handleCallResults: (callResults) => {
                        const rawSubCallResults = this._sampler._samplerContract.getABIDecodedReturnData('batchCall', callResults);
                        const decimals = ops.map((op, i) => op.handleCallResults(rawSubCallResults[i]));
                        return decimals;
                    },
                };
            })());
            const prices = tokens.map((t, i) => {
                if (!samplerResults[i]) {
                    return undefined;
                }
                const results = _.flatten(samplerResults[i]).filter(s => s && !s.output.isZero());
                const [buyToken, sellToken] = t;
                const [buyTokenDecimals, sellTokenDecimals] = decimals[i];
                return {
                    sellTokenDecimals,
                    buyTokenDecimals,
                    sellToken,
                    buyToken,
                    outputs: results.map(r => ({ output: r.output, fillData: r.fillData || {}, source: r.source })),
                    input: takerAmount,
                    ethToTakerRate,
                };
            });
    ...
    _generateOptimizedOrdersAsync(marketSideLiquidity, opts) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const takerAssetToEthRate = side === types_1.MarketOperation.Sell ? ethToInputRate : ethToOutputRate;
            const makerAssetToEthRate = side === types_1.MarketOperation.Sell ? ethToOutputRate : ethToInputRate;
            // Find the unoptimized best rate to calculate savings from optimizer
            const _unoptimizedPath = path_optimizer_1.fillsToSortedPaths(fills, side, inputAmount, optimizerOpts)[0];
    ...
    _getMarketSideOrdersAsync(nativeOrders, amount, side, opts) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let marketSideLiquidity = yield marketLiquidityFnAsync(nativeOrders, amount, _opts);
            let optimizerResult;
            try {
            ...
            if (rfqt && rfqt.quoteRequestor && marketSideLiquidity.quoteSourceFilters.isAllowed(types_2.ERC20BridgeSource.Native)) {
                ...
                if (rfqt.isIndicative && isIndicativePriceAwareEnabled) {
                    ...
                    if (indicativeQuotes.length > 0) {
                        marketSideLiquidity = Object.assign({}, marketSideLiquidity, { rfqtIndicativeQuotes: indicativeQuotes });
                        optimizerResult = yield this._generateOptimizedOrdersAsync(marketSideLiquidity, optimizerOpts);
                    }
                }
                    ...
                    if (rfqt.intentOnFilling) {
                        ...
                        if (firmQuotes.length > 0) {
                            ...
                            marketSideLiquidity = Object.assign({}, marketSideLiquidity, { nativeOrders: marketSideLiquidity.nativeOrders.concat(firmQuoteSignedOrders), orderFillableAmounts: marketSideLiquidity.orderFillableAmounts.concat(rfqOrderFillableAmounts) });
