...
var MarketOperationUtils = /** @class */ (function () {
            ...
            return __generator(this, function (_d) {
                switch (_d.label) {
                    case 0:
                        ...
                        samplerPromise = this._getSamplerResultsAsync(types_1.MarketOperation.Sell, nativeOrders, takerAmount, makerToken, takerToken, _opts);
            ...
            return __generator(this, function (_d) {
                switch (_d.label) {
                    case 0:
                        ...
                        _opts = __assign({}, constants_1.DEFAULT_GET_MARKET_ORDERS_OPTS, opts);
                        _a = __read(orders_1.getNativeOrderTokens(nativeOrders[0]), 2), makerToken = _a[0], takerToken = _a[1];
                        samplerPromise = this._getSamplerResultsAsync(types_1.MarketOperation.Buy, nativeOrders, makerAmount, makerToken, takerToken, _opts);
    ...
    MarketOperationUtils.prototype.getRawAmountsAsync = function (nativeOrders, takerAmount, opts) {
        return __awaiter(this, void 0, void 0, function () {
            var e_1, _a, e_2, _b, _opts, _c, makerToken, takerToken, samplerPromise, rfqtPromise, _d, _e, orderFillableAmounts, _liquidityProviderAddress, ethToMakerAssetRate, dexQuotes, rfqtIndicativeQuotes, paths, paths_1, paths_1_1, path, path_1, path_1_1, fill;
            return __generator(this, function (_f) {
                switch (_f.label) {
                    case 0:
                        ...
                        _opts = __assign({}, constants_1.DEFAULT_GET_MARKET_ORDERS_OPTS, opts);
                        _c = __read(orders_1.getNativeOrderTokens(nativeOrders[0]), 2), makerToken = _c[0], takerToken = _c[1];
                        samplerPromise = this._getSamplerResultsAsync(types_1.MarketOperation.Sell, nativeOrders, takerAmount, makerToken, takerToken, _opts);
                        rfqtPromise = getRfqtIndicativeQuotesAsync(nativeOrders[0].makerAssetData, nativeOrders[0].takerAssetData, types_1.MarketOperation.Sell, takerAmount, _opts);
                        return [4 /*yield*/, Promise.all([samplerPromise, rfqtPromise])];
                    case 1:
                        _d = __read.apply(void 0, [_f.sent(), 2]), _e = __read(_d[0], 4), orderFillableAmounts = _e[0], _liquidityProviderAddress = _e[1], ethToMakerAssetRate = _e[2], dexQuotes = _e[3], rfqtIndicativeQuotes = _d[1];
                        console.log(rfqtIndicativeQuotes);
                        paths = fills_1.createFillPaths({
                            // TODO handle BUY
                            side: types_1.MarketOperation.Sell,
                            // Augment native orders with their fillable amounts.
                            orders: __spread(orders_1.createSignedOrdersWithFillableAmounts(types_1.MarketOperation.Sell, nativeOrders, orderFillableAmounts), orders_1.createSignedOrdersFromRfqtIndicativeQuotes(rfqtIndicativeQuotes)),
                            dexQuotes: dexQuotes,
                        ...
                        try {
                            // Dex paths are not cumulative, but individual input/outputs
                            // We want cumulative input/outputs
                            for (paths_1 = __values(paths), paths_1_1 = paths_1.next(); !paths_1_1.done; paths_1_1 = paths_1.next()) {
                                path = paths_1_1.value;
                                // Don't do this for Native orders
                                if (path[0].source === types_2.ERC20BridgeSource.Native) {
                                    continue;
                                }
                                try {
                                    for (path_1 = __values(path), path_1_1 = path_1.next(); !path_1_1.done; path_1_1 = path_1.next()) {
                                        fill = path_1_1.value;
                                        fill.input = fill.input.plus(fill.parent ? fill.parent.input : 0);
                                        fill.output = fill.output.plus(fill.parent ? fill.parent.output : 0);
                                        // TODO handle BUY
                                        fill.rate = fill.output.div(fill.input);
                                    }
                                }
                        ...
                        return [2 /*return*/, paths.map(function (path) {
                                return path.map(function (fill) { return ({ input: fill.input, output: fill.output, source: fill.source, rate: fill.rate }); });
                            })];
                }
            });
        });
    ...
    MarketOperationUtils.prototype._generateOptimizedOrders = function (opts) {
        var inputToken = opts.inputToken, outputToken = opts.outputToken, side = opts.side, inputAmount = opts.inputAmount, getMarketOrdersOpts = opts.getMarketOrdersOpts;
        var maxFallbackSlippage = getMarketOrdersOpts.maxFallbackSlippage || 0;
        ...
        var paths = fills_1.createFillPaths({
            ...
            excludedSources: getMarketOrdersOpts.excludedSources,
            feeSchedule: getMarketOrdersOpts.feeSchedule,
        });
        // Find the optimal path.
        var optimalPath = path_optimizer_1.findOptimalPath(side, paths, inputAmount, getMarketOrdersOpts.runLimit) || [];
        ...
        if (getMarketOrdersOpts.allowFallback && nativeSubPath.length !== 0) {
            ...
            fallbackPath =
                path_optimizer_1.findOptimalPath(side, fills_1.getFallbackSourcePaths(optimalPath, paths), fallbackInputAmount, getMarketOrdersOpts.runLimit) || [];
        ...
        return orders_1.createOrdersFromPath(__spread(optimalPath, fallbackPath), {
            ...
            bridgeSlippage: getMarketOrdersOpts.bridgeSlippage || 0,
            liquidityProviderAddress: opts.liquidityProviderAddress,
            shouldBatchBridgeOrders: !!getMarketOrdersOpts.shouldBatchBridgeOrders,
        });
    };
    ...
    MarketOperationUtils.prototype._getSamplerResultsAsync = function (side, nativeOrders, amount, makerToken, takerToken, opts) {
        return __awaiter(this, void 0, void 0, function () {
            var operation;
            return __generator(this, function (_a) {
                operation = side === types_1.MarketOperation.Sell ? sampler_1.DexOrderSampler.ops.getSellQuotes : sampler_1.DexOrderSampler.ops.getBuyQuotes;
                // Call the sampler contract.
                return [2 /*return*/, this._sampler.executeAsync(
                    ...
                    // Get ETH -> maker token price for sell. Get ETH -> taker token price for buy.
                    sampler_1.DexOrderSampler.ops.getMedianSellRate(utils_2.difference(constants_1.FEE_QUOTE_SOURCES, opts.excludedSources).concat(this._liquidityProviderSourceIfAvailable(opts.excludedSources)), side === types_1.MarketOperation.Sell ? makerToken : takerToken, this._wethAddress, constants_1.ONE_ETHER, this._liquidityProviderRegistry), 
                    // Get quotes for taker -> maker.
                    operation(utils_2.difference(constants_1.SELL_SOURCES, opts.excludedSources).concat(this._liquidityProviderSourceIfAvailable(opts.excludedSources)), makerToken, takerToken, sampler_1.getSampleAmounts(amount, opts.numSamples, opts.sampleDistributionBase), this._liquidityProviderRegistry))];
            });
        });
    };
