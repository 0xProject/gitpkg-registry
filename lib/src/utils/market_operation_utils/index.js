...
class MarketOperationUtils {
    ...
    getMarketSellLiquidityAsync(nativeOrders, takerAmount, opts) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { onChain: sampleBalancerOnChain, offChain: sampleBalancerOffChain, } = this._sampler.balancerPoolsCache.howToSampleBalancer(takerToken, makerToken, _opts.excludedSources);
            // Call the sampler contract.
            const samplerPromise = this._sampler.executeAsync(
            // Get native order fillable amounts.
            this._sampler.getOrderFillableTakerAmounts(nativeOrders, this.contractAddresses.exchange), 
            // Get ETH -> maker token price.
            this._sampler.getMedianSellRate(utils_2.difference(constants_1.FEE_QUOTE_SOURCES.concat(this._optionalSources()), _opts.excludedSources), makerToken, this._wethAddress, constants_1.ONE_ETHER, this._wethAddress, this._liquidityProviderRegistry, this._multiBridge), 
            // Get ETH -> taker token price.
            this._sampler.getMedianSellRate(utils_2.difference(constants_1.FEE_QUOTE_SOURCES.concat(this._optionalSources()), _opts.excludedSources), takerToken, this._wethAddress, constants_1.ONE_ETHER, this._wethAddress, this._liquidityProviderRegistry, this._multiBridge), 
            // Get sell quotes for taker -> maker.
            this._sampler.getSellQuotes(utils_2.difference(constants_1.SELL_SOURCES.concat(this._optionalSources()), _opts.excludedSources.concat(sampleBalancerOnChain ? [] : types_2.ERC20BridgeSource.Balancer)), makerToken, takerToken, sampleAmounts, this._wethAddress, this._liquidityProviderRegistry, this._multiBridge), _opts.excludedSources.includes(types_2.ERC20BridgeSource.MultiHop)
                ? sampler_1.DexOrderSampler.constant([])
                : this._sampler.getTwoHopSellQuotes(utils_2.difference(constants_1.SELL_SOURCES.concat(this._optionalSources()), _opts.excludedSources.concat(types_2.ERC20BridgeSource.MultiBridge)), makerToken, takerToken, takerAmount, this._tokenAdjacencyGraph, this._wethAddress, this._liquidityProviderRegistry));
            ...
            const offChainBalancerPromise = sampleBalancerOffChain
                ...
                : Promise.resolve([]);
            const offChainBancorPromise = _opts.excludedSources.includes(types_2.ERC20BridgeSource.Bancor)
                ? Promise.resolve([])
    ...
    getMarketBuyLiquidityAsync(nativeOrders, makerAmount, opts) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this._sampler.getBuyQuotes(utils_2.difference(constants_1.BUY_SOURCES.concat(this._liquidityProviderRegistry !== utils_1.NULL_ADDRESS ? [types_2.ERC20BridgeSource.LiquidityProvider] : []), _opts.excludedSources.concat(sampleBalancerOnChain ? [] : types_2.ERC20BridgeSource.Balancer)), makerToken, takerToken, sampleAmounts, this._wethAddress, this._liquidityProviderRegistry), _opts.excludedSources.includes(types_2.ERC20BridgeSource.MultiHop)
                ? sampler_1.DexOrderSampler.constant([])
                : this._sampler.getTwoHopBuyQuotes(utils_2.difference(constants_1.BUY_SOURCES.concat(this._liquidityProviderRegistry !== utils_1.NULL_ADDRESS
                    ? [types_2.ERC20BridgeSource.LiquidityProvider]
                    : []), _opts.excludedSources), makerToken, takerToken, makerAmount, this._tokenAdjacencyGraph, this._wethAddress, this._liquidityProviderRegistry));
            const offChainBalancerPromise = sampleBalancerOffChain
    ...
    getBatchMarketBuyOrdersAsync(batchNativeOrders, makerAmounts, opts) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const _opts = Object.assign({}, constants_1.DEFAULT_GET_MARKET_ORDERS_OPTS, opts);
            const sources = utils_2.difference(constants_1.BUY_SOURCES, _opts.excludedSources.concat(types_2.ERC20BridgeSource.Balancer));
            const ops = [
                ...batchNativeOrders.map(orders => this._sampler.getOrderFillableMakerAmounts(orders, this.contractAddresses.exchange)),
                ...batchNativeOrders.map(orders => this._sampler.getMedianSellRate(utils_2.difference(constants_1.FEE_QUOTE_SOURCES, _opts.excludedSources), orders_1.getNativeOrderTokens(orders[0])[1], this._wethAddress, constants_1.ONE_ETHER, this._wethAddress)),
                ...batchNativeOrders.map((orders, i) => this._sampler.getBuyQuotes(sources, orders_1.getNativeOrderTokens(orders[0])[0], orders_1.getNativeOrderTokens(orders[0])[1], [makerAmounts[i]], this._wethAddress)),
            ];
    ...
    _generateOptimizedOrdersAsync(marketSideLiquidity, opts) {
        return __awaiter(this, void 0, void 0, function* () {
            const { inputToken, outputToken, side, inputAmount, nativeOrders, orderFillableAmounts, rfqtIndicativeQuotes, dexQuotes, ethToOutputRate, ethToInputRate, twoHopQuotes, } = marketSideLiquidity;
            ...
            const { rate: bestTwoHopRate, quote: bestTwoHopQuote } = multihop_utils_1.getBestTwoHopQuote(marketSideLiquidity, opts.feeSchedule);
            if (bestTwoHopRate.isGreaterThan(optimalPathRate)) {
                ...
                const twoHopQuoteReport = quote_report_generator_1.generateQuoteReport(side, _.flatten(dexQuotes), twoHopQuotes, nativeOrders, orderFillableAmounts, bestTwoHopQuote, opts.quoteRequestor);
            ...
            const quoteReport = quote_report_generator_1.generateQuoteReport(side, _.flatten(dexQuotes), twoHopQuotes, nativeOrders, orderFillableAmounts, _.flatten(optimizedOrders.map(order => order.fills)), opts.quoteRequestor);
