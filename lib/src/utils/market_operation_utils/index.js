...
class MarketOperationUtils {
    ...
    _generateOptimizedOrdersAsync(marketSideLiquidity, opts) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const ethToTakerAssetRate = side === types_1.MarketOperation.Sell ? ethToInputRate : ethToOutputRate;
            const ethToMakerAssetRate = side === types_1.MarketOperation.Sell ? ethToOutputRate : ethToInputRate;
            // Find the unoptimized best rate to calculate savings from optimizer
            const unoptimizedPath = path_optimizer_1.fillsToSortedPaths(fills, side, inputAmount, optimizerOpts)[0].collapse(orderOpts);
            // Find the optimal path
