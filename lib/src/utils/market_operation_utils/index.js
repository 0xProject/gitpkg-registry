...
class MarketOperationUtils {
    ...
    getPricesAsync(nativeOrders, makerAmounts, opts) {
        return __awaiter(this, void 0, void 0, function* () {
            if (nativeOrders.length === 0) {
                throw new Error(types_2.AggregationError.EmptyOrders);
            }
            ...
            const ops = _.flatten([
                this._sampler.getMedianSellRate(feeSourceFilters.sources, tokens[0][1], this._wethAddress, constants_1.ONE_ETHER),
                ..._.flatten(nativeOrders.map((_order, i) => {
                    return this._sampler.getBuyQuotes(quoteSourceFilters.sources, tokens[i][0], tokens[i][1], [
                        makerAmounts[i],
                    ]);
                })),
            ]);
            const [ethToTakerRate, ...samplerResults] = yield this._sampler.executeBatchAsync(ops);
            const prices = nativeOrders.map((_o, i) => {
                if (!samplerResults[i] || !samplerResults[i][0]) {
                    return undefined;
                }
                const results = samplerResults[i]
                    .map((s) => s[0])
                    .filter((s) => s && !s.output.isZero());
                const [buyToken, sellToken] = tokens[i];
                return {
                    sellToken,
                    buyToken,
                    outputs: results.map(r => ({ output: r.output, fillData: r.fillData || {}, source: r.source })),
                    input: makerAmounts[i],
                    ethToTakerRate,
                };
            });
            return prices;
