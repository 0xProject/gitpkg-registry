...
var MarketOperationUtils = /** @class */ (function () {
            ...
            return __generator(this, function (_d) {
                switch (_d.label) {
                    case 0:
                        ...
                        sampler_1.DexOrderSampler.ops.getMedianSellRate(utils_2.difference(constants_1.FEE_QUOTE_SOURCES.concat(this._optionalSources()), _opts.excludedSources), makerToken, this._wethAddress, constants_1.ONE_ETHER, this._wethAddress, this._liquidityProviderRegistry, this._multiBridge), 
                        // Get sell quotes for taker -> maker.
                        sampler_1.DexOrderSampler.ops.getSellQuotes(utils_2.difference(constants_1.SELL_SOURCES.concat(this._optionalSources()), _opts.excludedSources), makerToken, takerToken, sampler_1.getSampleAmounts(takerAmount, _opts.numSamples, _opts.sampleDistributionBase), this._wethAddress, this._liquidityProviderRegistry, this._multiBridge));
            ...
            return __generator(this, function (_d) {
                switch (_d.label) {
                    case 0:
                        ...
                        sampler_1.DexOrderSampler.ops.getMedianSellRate(utils_2.difference(constants_1.FEE_QUOTE_SOURCES.concat(this._optionalSources()), _opts.excludedSources), takerToken, this._wethAddress, constants_1.ONE_ETHER, this._wethAddress, this._liquidityProviderRegistry, this._multiBridge), 
                        // Get buy quotes for taker -> maker.
                        sampler_1.DexOrderSampler.ops.getBuyQuotes(utils_2.difference(constants_1.BUY_SOURCES.concat(this._liquidityProviderRegistry !== utils_1.NULL_ADDRESS ? [types_2.ERC20BridgeSource.LiquidityProvider] : []), _opts.excludedSources), makerToken, takerToken, sampler_1.getSampleAmounts(makerAmount, _opts.numSamples, _opts.sampleDistributionBase), this._wethAddress, this._liquidityProviderRegistry));
                ...
                switch (_a.label) {
                    case 0:
                        ...
                        ops = __spread(batchNativeOrders.map(function (orders) { return sampler_1.DexOrderSampler.ops.getOrderFillableMakerAmounts(orders); }), batchNativeOrders.map(function (orders) {
                            return sampler_1.DexOrderSampler.ops.getMedianSellRate(utils_2.difference(constants_1.FEE_QUOTE_SOURCES, _opts.excludedSources), orders_1.getNativeOrderTokens(orders[0])[1], _this._wethAddress, constants_1.ONE_ETHER, _this._wethAddress);
                        }), batchNativeOrders.map(function (orders, i) {
                            return sampler_1.DexOrderSampler.ops.getBuyQuotes(sources, orders_1.getNativeOrderTokens(orders[0])[0], orders_1.getNativeOrderTokens(orders[0])[1], [makerAmounts[i]], _this._wethAddress);
                        }));
    ...
    MarketOperationUtils.prototype._generateOptimizedOrders = function (opts) {
        ...
        if (opts.allowFallback && nativeSubPath.length !== 0) {
            ...
            var nonNativePaths = paths.filter(function (p) { return p.length > 0 && p[0].source !== types_2.ERC20BridgeSource.Native; });
            var nonNativeOptimalPath = path_optimizer_1.findOptimalPath(side, nonNativePaths, inputAmount, opts.runLimit) || [];
            // Calculate the slippage of on-chain sources compared to the most optimal path
            var fallbackSlippage = fills_1.getPathAdjustedSlippage(side, nonNativeOptimalPath, inputAmount, fills_1.getPathAdjustedRate(side, optimalPath, inputAmount));
            if (nativeSubPath.length === optimalPath.length || fallbackSlippage <= maxFallbackSlippage) {
                ...
                var _a = __read(optimalPath.slice().reverse(), 2), last = _a[0], penultimateIfExists = _a[1];
                var lastNativeFillIfExists_1 = last.source === types_2.ERC20BridgeSource.Native &&
                    penultimateIfExists &&
        ...
        return orders_1.createOrdersFromPath(optimalPath, {
            side: side,
