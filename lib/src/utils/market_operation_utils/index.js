...
class MarketOperationUtils {
    ...
    static _computeExtendedQuoteReportSources(quoteRequestor, marketSideLiquidity, amount, optimizerResult, comparisonPrice) {
        const { side, quotes } = marketSideLiquidity;
        const { liquidityDelivered } = optimizerResult;
        return quote_report_generator_1.generateExtendedQuoteReportSources(side, quotes, liquidityDelivered, amount, comparisonPrice, quoteRequestor);
    }
    ...
    getMarketSellLiquidityAsync(nativeOrders, takerAmount, opts) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const samplerPromise = this._sampler.executeAsync(this._sampler.getBlockNumber(), this._sampler.getGasLeft(), this._sampler.getTokenDecimals([makerToken, takerToken]), 
            // Get native order fillable amounts.
            this._sampler.getLimitOrderFillableTakerAmounts(nativeOrders, this.contractAddresses.exchangeProxy), 
            // Get ETH -> maker token price.
            // Set the global sample value to discover the network fee token in maker token
            this._sampler.setSampleValues([this._nativeFeeTokenAmount]), this._sampler.getMedianSellRate(feeSourceFilters.sources, makerToken, this._nativeFeeToken, this._nativeFeeTokenAmount), 
            ...
            this._sampler.setSampleValues(sampleAmounts), this._sampler.getSellQuotes(quoteSourceFilters.sources, makerToken, takerToken, sampleAmounts), 
            // Set the global sample values to just the entire amount for MultiHop
            this._sampler.setSampleValues([takerAmount]), this._sampler.getTwoHopSellQuotes(quoteSourceFilters.isAllowed(types_2.ERC20BridgeSource.MultiHop) ? quoteSourceFilters.sources : [], makerToken, takerToken, takerAmount), this._sampler.isAddressContract(txOrigin), this._sampler.getGasLeft());
            // Refresh the cached pools asynchronously if required
            void this._refreshPoolCacheIfRequiredAsync(takerToken, makerToken);
            const [[blockNumber, gasBefore, tokenDecimals, orderFillableTakerAmounts, _setFeeTokenSample, outputAmountPerEth, inputAmountPerEth, _setSampleValues, dexQuotes, _setMultiHopSample, rawTwoHopQuotes, isTxOriginContract, gasAfter,],] = yield Promise.all([samplerPromise]);
            // Log the gas metrics
    ...
    _generateOptimizedOrdersAsync(marketSideLiquidity, opts) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (SHOULD_USE_RUST_ROUTER) {
                optimalPath = path_optimizer_1.findOptimalRustPathFromSamples(side, dexQuotes, [...nativeOrders, ...augmentedRfqtIndicativeQuotes], inputAmount, penaltyOpts, opts.feeSchedule, this._sampler.chainId, opts.neonRouterNumSamples, opts.samplerMetrics);
            }
            else {
                optimalPath = yield path_optimizer_1.findOptimalPathJSAsync(side, fills, inputAmount, opts.runLimit, opts.samplerMetrics, penaltyOpts);
            }
            ...
            yield this._addOptionalFallbackAsync(side, inputAmount, optimalPath, dexQuotes, fills, opts, penaltyOpts);
    ...
    _addOptionalFallbackAsync(side, inputAmount, optimalPath, dexQuotes, fills, opts, penaltyOpts) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (opts.allowFallback && fragileFills.length !== 0) {
                ...
                const sturdyPenaltyOpts = Object.assign(Object.assign({}, penaltyOpts), { exchangeProxyOverhead: (sourceFlags) => 
                    // tslint:disable-next-line: no-bitwise
                    penaltyOpts.exchangeProxyOverhead(sourceFlags | optimalPath.sourceFlags) });
                let sturdyOptimalPath;
                if (SHOULD_USE_RUST_ROUTER) {
                    const sturdySamples = dexQuotes.filter(samples => samples.length > 0 && !fragileSources.includes(samples[0].source));
                    sturdyOptimalPath = path_optimizer_1.findOptimalRustPathFromSamples(side, sturdySamples, [], inputAmount, sturdyPenaltyOpts, opts.feeSchedule, this._sampler.chainId, opts.neonRouterNumSamples, undefined);
                }
                else {
                    const sturdyFills = fills.filter(p => p.length > 0 && !fragileSources.includes(p[0].source));
                    sturdyOptimalPath = yield path_optimizer_1.findOptimalPathJSAsync(side, sturdyFills, inputAmount, opts.runLimit, undefined, // hack: set sampler metrics to undefined to avoid fallback timings
                    sturdyPenaltyOpts);
                }
