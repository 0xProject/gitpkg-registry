...
function createFills(opts) {
    ...
    // Create DEX fills.
    const dexFills = dexQuotes.map(singleSourceSamples => dexSamplesToFills(side, singleSourceSamples, outputAmountPerEth, inputAmountPerEth, feeSchedule, opts.gasPrice));
...
function dexSamplesToFills(side, samples, outputAmountPerEth, inputAmountPerEth, fees, gasPrice) {
    const sourcePathId = utils_1.hexUtils.random();
    const fills = [];
