...
function createFills(opts) {
    ...
    // Create native fills.
    const nativeFills = nativeOrdersToFills(side, orders, opts.targetInput, ethToOutputRate, ethToInputRate, feeSchedule);
    // Create DEX fills.
    const dexFills = dexQuotes.map(singleSourceSamples => dexSamplesToFills(side, singleSourceSamples, ethToOutputRate, ethToInputRate, feeSchedule));
...
function nativeOrdersToFills(side, orders, targetInput = constants_1.POSITIVE_INF, ethToOutputRate, ethToInputRate, fees) {
    ...
    for (const order of orders) {
        ...
        fills.push({
            ...
            flags: constants_1.SOURCE_FLAGS[types_2.ERC20BridgeSource.Native],
            index: 0,
            parent: undefined,
...
function dexSamplesToFills(side, samples, ethToOutputRate, ethToInputRate, fees) {
    const sourcePathId = utils_1.hexUtils.random();
    const fills = [];
