...
function nativeOrdersToFills(side, orders, targetInput = constants_1.POSITIVE_INF, outputAmountPerEth, inputAmountPerEth, gasPrice) {
    ...
    for (const o of orders) {
        ...
        const gasUsed = o.type === protocol_utils_1.FillQuoteTransformerOrderType.Limit ? constants_1.NATIVE_LIMIT_ORDER_GAS_USED : constants_1.NATIVE_RFQT_GAS_USED;
...
function dexSamplesToFills(side, samples, outputAmountPerEth, inputAmountPerEth, gasPrice) {
    ...
    for (let i = 0; i < nonzeroSamples.length; i++) {
        ...
        const adjustedRate = side === types_1.MarketOperation.Sell ? adjustedOutput.div(input) : input.div(adjustedOutput);
        fills.push({
            sourcePathId,
            input,
            output,
