...
function nativeOrdersToFills(side, orders, targetInput = constants_1.POSITIVE_INF, outputAmountPerEth, inputAmountPerEth, fees) {
    ...
    for (const o of orders) {
        ...
        const input = side === types_2.MarketOperation.Sell ? takerAmount : makerAmount;
        const output = side === types_2.MarketOperation.Sell ? makerAmount : takerAmount;
        const fee = fees[types_1.ERC20BridgeSource.Native] === undefined ? 0 : fees[types_1.ERC20BridgeSource.Native](o);
        ...
        const adjustedOutput = side === types_2.MarketOperation.Sell ? clippedOutput.minus(outputPenalty) : clippedOutput.plus(outputPenalty);
        const adjustedRate = side === types_2.MarketOperation.Sell ? adjustedOutput.div(clippedInput) : clippedInput.div(adjustedOutput);
...
function dexSamplesToFills(side, samples, outputAmountPerEth, inputAmountPerEth, fees) {
    ...
    for (let i = 0; i < nonzeroSamples.length; i++) {
        ...
        const adjustedOutput = side === types_2.MarketOperation.Sell ? output.minus(penalty) : output.plus(penalty);
        fills.push({
            sourcePathId,
            input,
            output,
