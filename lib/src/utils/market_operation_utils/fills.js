...
function createFillPaths(opts) {
    ...
    var nativePath = nativeOrdersToPath(side, orders, ethToOutputRate, feeSchedule);
...
function nativeOrdersToPath(side, orders, ethToOutputRate, fees) {
    var e_1, _a;
    ...
    try {
        for (var orders_1 = __values(orders), orders_1_1 = orders_1.next(); !orders_1_1.done; orders_1_1 = orders_1.next()) {
            ...
            var penalty = ethToOutputRate.times(fees[types_2.ERC20BridgeSource.Native] || 0);
            ...
            var adjustedRate = side === types_1.MarketOperation.Sell
...
function sourceToFillFlags(source) {
    if (source === types_2.ERC20BridgeSource.Kyber) {
        return types_2.FillFlags.Kyber;
    }
    if (source === types_2.ERC20BridgeSource.Eth2Dai) {
        return types_2.FillFlags.ConflictsWithKyber;
    }
...
function isValidPath(path) {
    ...
    var conflictFlags = types_2.FillFlags.Kyber | types_2.FillFlags.ConflictsWithKyber;
...
function collapsePath(side, path) {
    var e_6, _a;
    var collapsed = [];
    try {
        for (var path_4 = __values(path), path_4_1 = path_4.next(); !path_4_1.done; path_4_1 = path_4.next()) {
            var fill = path_4_1.value;
            var makerAssetAmount = side === types_1.MarketOperation.Sell ? fill.output : fill.input;
            var takerAssetAmount = side === types_1.MarketOperation.Sell ? fill.input : fill.output;
            ...
            collapsed.push({
                ...
                nativeOrder: fill.source === types_2.ERC20BridgeSource.Native ? fill.fillData.order : undefined,
            });
        }
    }
