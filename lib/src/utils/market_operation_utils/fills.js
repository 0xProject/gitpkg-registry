...
function getPathAdjustedRate(side, path, targetInput) {
    var _a = __read(getPathAdjustedSize(path, targetInput), 2), output = _a[1];
    if (output.eq(0)) {
        return constants_1.ZERO_AMOUNT;
    }
    return side === types_1.MarketOperation.Sell ? output.div(targetInput) : targetInput.div(output);
}
