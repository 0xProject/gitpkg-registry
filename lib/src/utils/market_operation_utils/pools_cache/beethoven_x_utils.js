...
exports.BeethovenXPoolsCache = void 0;
...
class BeethovenXPoolsCache extends pools_cache_1.PoolsCache {
    constructor(chainId, subgraphUrl = constants_2.BEETHOVEN_X_SUBGRAPH_URL_BY_CHAIN[chainId], maxPoolsFetched = constants_2.BALANCER_MAX_POOLS_FETCHED, _topPoolsFetched = constants_2.BALANCER_TOP_POOLS_FETCHED, _warningLogger = constants_1.DEFAULT_WARNING_LOGGER, cache = {}) {
        super(cache);
                ...
                for (const from of tokensList) {
                    for (const to of tokensList.filter(t => t.toLowerCase() !== from.toLowerCase())) {
                        ...
                        try {
                            ...
                            fromToPools[from][to].push(BeethovenXPoolsCache._parseSubgraphPoolData(poolData[pool.id], from, to));
                            // Cache this as we progress through
                            const expiresAt = Date.now() + this._cacheTimeMs;
                            this._cachePoolsForPair(from, to, fromToPools[from][to], expiresAt);
                        }
                        catch (err) {
                            this._warningLogger(err, `Failed to load Beethoven X top pools`);
                            // soldier on
                        }
                    }
                }
    ...
    _fetchPoolsForPairAsync(takerToken, makerToken) {
        ...
        query getPools {
          ...
          `;
            try {
                const { pools } = yield graphql_request_1.request(this.subgraphUrl, query);
                return pools.map((pool) => BeethovenXPoolsCache._parseSubgraphPoolData(pool, takerToken, makerToken));
            }
            catch (e) {
                return [];
            }
...
exports.BeethovenXPoolsCache = BeethovenXPoolsCache;
//# sourceMappingURL=beethoven_x_utils.js.map