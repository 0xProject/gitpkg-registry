...
function simulateBestCaseFill(quoteInfo) {
    const opts = Object.assign(Object.assign({}, DEFAULT_SIMULATED_FILL_QUOTE_INFO_OPTS), quoteInfo.opts);
    const protocolFeePerFillOrder = quoteInfo.gasPrice.times(opts.protocolFeeMultiplier);
    const result = fillQuoteOrders(createBestCaseFillOrderCalls(quoteInfo), quoteInfo.fillAmount, protocolFeePerFillOrder);
...
function simulateWorstCaseFill(quoteInfo) {
    ...
    const result = Object.assign(Object.assign({}, fillQuoteOrders(bestCase, quoteInfo.fillAmount, protocolFeePerFillOrder)), { 
        // Worst case gas and protocol fee is hitting all orders.
        gas: getTotalGasUsedByFills(quoteInfo.orders), protocolFee: protocolFeePerFillOrder.times(quoteInfo.orders.filter(o => hasProtocolFee(o)).length) });
...
function fillQuoteOrders(fillOrders, inputAmount, protocolFeePerFillOrder) {
    const result = Object.assign(Object.assign({}, EMPTY_QUOTE_INTERMEDIATE_FILL_RESULT), { inputBySource: {} });
    ...
    for (const fo of fillOrders) {
        ...
        for (const fill of fo.order.fills) {
            ...
            result.gas += new utils_1.BigNumber(gasUsed).toNumber();
    ...
    return orders.map(o => (Object.assign({ order: o }, (side === types_1.MarketOperation.Sell
        ? {
            ...
            totalOrderInputFee: o.type === protocol_utils_1.FillQuoteTransformerOrderType.Limit
                ? utils_2.getNativeAdjustedTakerFeeAmount(o.fillData.order, o.takerAmount)
                : ZERO_AMOUNT,
...
function fromIntermediateQuoteFillResult(ir, quoteInfo) {
    ...
    return Object.assign(Object.assign({}, (side === types_1.MarketOperation.Sell
        ? // Sell
            {
...
function getTotalGasUsedByFills(fills) {
    let gasUsed = 0;
    for (const f of fills) {
        gasUsed += new utils_1.BigNumber(f.gasUsed).toNumber();
    }
    return gasUsed;
}
