...
function simulateWorstCaseFill(quoteInfo) {
    ...
    const result = Object.assign(Object.assign({}, fillQuoteOrders(bestCase, quoteInfo.fillAmount, protocolFeePerFillOrder, opts.gasSchedule)), { 
        // Worst case gas and protocol fee is hitting all orders.
        gas: getTotalGasUsedByFills(quoteInfo.orders, opts.gasSchedule), protocolFee: protocolFeePerFillOrder.times(quoteInfo.orders.filter(o => hasProtocolFee(o)).length) });
    ...
    const outputMultiplier = quoteInfo.side === types_1.MarketOperation.Sell
        ? new utils_1.BigNumber(1).minus(opts.slippage)
        : new utils_1.BigNumber(1).plus(opts.slippage);
...
function fillQuoteOrders(fillOrders, inputAmount, protocolFeePerFillOrder, gasSchedule) {
    ...
    for (const fo of fillOrders) {
        ...
        for (const fill of fo.order.fills) {
            ...
            result.gas += new utils_1.BigNumber(gas).toNumber();
