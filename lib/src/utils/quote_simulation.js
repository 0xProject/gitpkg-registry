...
function simulateWorstCaseFill(quoteInfo) {
    ...
    const result = Object.assign({}, fillQuoteOrders(
    // TODO jacob
    // createWorstCaseFillOrderCalls(quoteInfo),
    bestCase, quoteInfo.fillAmount, protocolFeePerFillOrder, opts.gasSchedule), { 
    ...
    return orders.map(o => (Object.assign({ order: o }, (side === types_1.MarketOperation.Sell
        ? {
            ...
            totalOrderInputFee: o.type === protocol_utils_1.FillQuoteTransformerOrderType.Limit
                ? o.fillData.order.takerTokenFeeAmount
                : ZERO_AMOUNT,
...
function createWorstCaseFillOrderCalls(quoteInfo) {
    ...
    return (createBestCaseFillOrderCalls(quoteInfo)
        .map(fo => (Object.assign({}, fo)))
        // Sort by ascending price.
        .sort((a, b) => a.order.makerAmount.div(a.order.takerAmount).comparedTo(b.order.makerAmount.div(b.order.takerAmount))));
}
