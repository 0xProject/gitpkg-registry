...
function getSlippedOrderFills(order, side) {
    ...
    if (source === types_2.ERC20BridgeSource.Native) {
        ...
        inputScaling = new utils_1.BigNumber(1);
        outputScaling = new utils_1.BigNumber(1);
    }
    else {
        if (side === types_1.MarketOperation.Sell) {
            const totalFillableTakerAssetAmount = utils_1.BigNumber.sum(...order.fills.map(f => f.input));
            const totalFillableMakerAssetAmount = utils_1.BigNumber.sum(...order.fills.map(f => f.output));
            ...
            const totalFillableTakerAssetAmount = utils_1.BigNumber.sum(...order.fills.map(f => f.output));
            const totalFillableMakerAssetAmount = utils_1.BigNumber.sum(...order.fills.map(f => f.input));
