...
exports.getFlattenedFillsFromOrders = exports.fillQuoteOrders = exports.simulateWorstCaseFill = exports.simulateBestCaseFill = void 0;
...
function simulateBestCaseFill(quoteInfo) {
    var opts = __assign(__assign({}, DEFAULT_SIMULATED_FILL_QUOTE_INFO_OPTS), quoteInfo.opts);
...
function simulateWorstCaseFill(quoteInfo) {
    ...
    var result = __assign(__assign({}, fillQuoteOrders(createWorstCaseFillOrderCalls(quoteInfo), quoteInfo.fillAmount, protocolFeePerFillOrder, opts.gasSchedule)), { 
...
function fillQuoteOrders(fillOrders, inputAmount, protocolFeePerFillOrder, gasSchedule) {
    ...
    var result = __assign(__assign({}, EMPTY_QUOTE_INTERMEDIATE_FILL_RESULT), { inputBySource: {} });
    ...
    try {
        for (var fillOrders_1 = __values(fillOrders), fillOrders_1_1 = fillOrders_1.next(); !fillOrders_1_1.done; fillOrders_1_1 = fillOrders_1.next()) {
            ...
            try {
                for (var _d = (e_2 = void 0, __values(fo.order.fills)), _e = _d.next(); !_e.done; _e = _d.next()) {
                    var fill = _e.value;
                    if (remainingInput.lte(0)) {
                        break;
                    }
                    var source = fill.source, fillData = fill.fillData;
                    var fee = gasSchedule[source] === undefined ? 0 : gasSchedule[source](fillData);
                    ...
                    try {
                        ...
                        for (var _f = (e_3 = void 0, __values(fill.subFills)), _g = _f.next(); !_g.done; _g = _f.next()) {
...
function createWorstCaseFillOrderCalls(quoteInfo) {
    ...
    return (createBestCaseFillOrderCalls(quoteInfo)
        .map(function (fo) { return (__assign(__assign({}, fo), { order: __assign(__assign({}, fo.order), { 
            // Apply slippage to order fills and reverse them.
            fills: getSlippedOrderFills(fo.order, quoteInfo.side)
                .map(function (f) { return (__assign(__assign({}, f), { subFills: f.subFills.slice().reverse() })); })
                .reverse() }) })); })
        // Sort by ascending price.
        .sort(function (a, b) {
        return a.order.makerAssetAmount
...
function getSlippedOrderFills(order, side) {
    ...
    return order.fills.map(function (f) { return (__assign(__assign({}, f), { input: f.input.times(inputScaling), output: f.output.times(outputScaling), subFills: f.subFills.map(function (sf) { return (__assign(__assign({}, sf), { input: sf.input.times(inputScaling), output: sf.output.times(outputScaling) })); }) })); });
}
...
function roundIntermediateFillResult(ir, side) {
    return {
        ...
        inputBySource: Object.assign.apply(Object, __spread([{}], Object.entries(ir.inputBySource).map(function (_a) {
            ...
            return (_b = {}, _b[k] = roundInputAmount(v, side), _b);
        }))),
    };
}
function fromIntermediateQuoteFillResult(ir, quoteInfo) {
    ...
    return __assign(__assign({}, (side === types_1.MarketOperation.Sell
        ? // Sell
            {
...
function getTotalGasUsedByFills(fills, gasSchedule) {
    ...
    try {
        for (var fills_1 = __values(fills), fills_1_1 = fills_1.next(); !fills_1_1.done; fills_1_1 = fills_1.next()) {
            var f = fills_1_1.value;
            var fee = gasSchedule[f.source] === undefined ? 0 : gasSchedule[f.source](f.fillData);
