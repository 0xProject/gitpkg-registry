...
function createContractWrapperAndHelper(chain, contractType, artifactName, address) {
    ...
    if (!address || address.toLowerCase() === detAddress) {
        wrapper = CONTRACT_WRAPPER_CACHE[artifactName];
    }
    if (!wrapper) {
        wrapper = new contractType(address || detAddress, constants_1.DUMMY_PROVIDER, {}, {}, !address ? artifact.compilerOutput.evm.deployedBytecode.object : undefined, fast_abi_1.createFastAbiEncoderOverrides(contractType));
    }
    if (!address) {
