...
class LiveChain {
    ...
    _dispatchBatchAsync(calls) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let rawResultData;
            try {
                rawResultData = yield this._w3.callAsync(LiveChain._mergeBatchCalls(calls), 12834530);
            }
            catch (err) {
...
function canBatchCallWith(ethCall, batch) {
    ...
    if (batch.length >= 96) { // TODO: Make this number configurable.
        return false;
    }
    ...
    // Split if the estimated RPC payload size would be too large.
    {
        ...
        if (estimatedRpcSize >= 800e3) { // TODO: Make this number configurable.
            return false;
        }
    }
