...
class LiveChain {
    constructor(opts) {
        ...
        this._flushThreshold = opts.emergencyFlushThreshold;
        this._minDispatchFullness = opts.minDispatchFullness;
        this._maxPendingCallAgeMs = opts.maxPendingCallAgeMs;
        this._maxCallsPerFlush = opts.maxCallsPerFlush;
    }
    get provider() {
    ...
    static createAsync(opts) {
        return __awaiter(this, void 0, void 0, function* () {
            const fullOpts = Object.assign({ maxCacheAgeMs: 10e3, pruneFrequencyMs: 5e3, 
                // These numbers will all affect how calls are batched across
                // RPC calls.
                flushFrequencyMs: 100, maxBatchGas: 1.5e9, maxBatchBytes: 8e6, emergencyFlushThreshold: 4, minDispatchFullness: 0.75, maxPendingCallAgeMs: 100, maxCallsPerFlush: 3 }, opts);
            ...
            const inst = new LiveChain(Object.assign({ chainId,
                w3 }, fullOpts));
            inst._flush(fullOpts.flushFrequencyMs);
    ...
    ethCallAsync(opts) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (opts.immediate) {
                ...
                if (this._queueFullness >= this._flushThreshold) {
                    this._flushQueue();
                }
            }
            return p;
    ...
    _flush(frequency) {
        ...
        this._flushQueue();
        setTimeout(() => __awaiter(this, void 0, void 0, function* () { return this._flush(frequency); }), Math.max(0, nextFlushTime - Date.now()));
    }
    _prune(frequency) {
    ...
    _flushQueue() {
        const batches = this._generateCallBatches(this._maxCallsPerFlush);
        for (const b of batches) {
            void this._dispatchBatchAsync(b);
        }
    }
    _pruneCache() {
        const now = Date.now();
    ...
    _dispatchBatchAsync(calls) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                rawResultData = yield utils_2.timeItAsync(() => this._w3.callAsync(mergedCalls), dt => `callAsync took ${dt}ms`);
            }
            catch (err) {
            ...
            const [results, blockNumber] = utils_2.timeIt(() => DISPATCHER_CONTRACT.getABIDecodedReturnData('dispatch', rawResultData), dt => `dispatch ABI decode took ${dt}ms`);
    ...
    _generateCallBatches(maxBatches = 8) {
        const batch = [];
        // Sort queue by ascending creation time.
        let nextQueue = this._queue.sort((a, b) => a.createdTime - b.createdTime);
        while (batch.length < maxBatches && nextQueue.length > 0) {
            ...
            if (batcher.maxAgeMs < this._maxPendingCallAgeMs && batcher.fullness < this._minDispatchFullness) {
                // Batch isn't worth dispatching and there isn't an old call in it
                // so return the calls to the queue and stop.
                nextQueue.push(...batcher.calls);
                break;
            }
            this._queueFullness -= batcher.fullness;
...
class CallBatcher {
    ...
    tryToBatch(ethCall) {
        ...
        this.fullness += getCallFullness(ethCall, this._maxBatchGas, this._maxBatchBytes);
        this.maxAgeMs = Math.max(Date.now() - ethCall.createdTime, this.maxAgeMs);
...
function mergeBatchCalls(calls) {
    return utils_2.timeIt(() => {
        ...
        return Object.assign(Object.assign({}, merged), { data: utils_2.timeIt(() => DISPATCHER_CONTRACT.dispatch(callInfos).getABIEncodedTransactionData(), dt => `dispatch ABI encode took ${dt}ms (${calls.length} calls)`), gasPrice: merged.gasPrice || constants_1.ZERO_AMOUNT });
