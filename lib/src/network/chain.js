...
class LiveChain {
    ...
    static createAsync(opts) {
        return __awaiter(this, void 0, void 0, function* () {
            const fullOpts = Object.assign({ maxCacheAgeMs: 600e3, pruneFrequencyMs: 5e3, callTimeoutMs: 5e3, 
                ...
                // RPC calls.
                flushFrequencyMs: 100, maxBatchGas: 2e9, maxBatchBytes: 0.512e6, targetFullness: 1, minOutstandingCalls: 5 }, opts);
            ...
            inst._autoPrune(fullOpts.pruneFrequencyMs);
            return inst;
        });
    }
    ethCallAsync(opts) {
    ...
    _autoPrune(frequency) {
        utils_2.timeIt(() => this._pruneCache(), dt => `pruneCache took ${dt}ms`);
        setTimeout(() => __awaiter(this, void 0, void 0, function* () { return this._autoPrune(frequency); }), frequency);
    }
    _pruneCache() {
        const now = Date.now();
    ...
    _generateCallBatches(maxBatches) {
        const batch = [];
...
class CallBatcher {
    constructor(_maxBatchGas, _maxBatchBytes, _batchId) {
    ...
    tryToBatch(ethCall) {
        var _a, _b;
        // Batch ids must be the same if set.
        if (this._batchId && this._batchId !== ethCall.opts.batchId) {
            return false;
        }
        ...
        this._batchId = this._batchId || ethCall.opts.batchId;
