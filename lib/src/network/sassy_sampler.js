...
class SassySampler {
    ...
    getTwoHopSellSamplesAsync(sources, takerToken, makerToken, takerAmount) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const hopResults = yield Promise.all(tokenPaths.map((tokenPath) => __awaiter(this, void 0, void 0, function* () {
                return this._twoHopSampler.getTwoHopSellSampleAsync(this._filterTwoHopSourcesByLiquidityChance(sources, tokenPath), tokenPath, takerAmount);
            })));
    ...
    getTwoHopBuySamplesAsync(sources, takerToken, makerToken, makerAmount) {
        return __awaiter(this, void 0, void 0, function* () {
            const tokenPaths = this._getTwoHopTokenPaths(takerToken, makerToken);
            const hopResults = yield Promise.all(tokenPaths.map((tokenPath) => __awaiter(this, void 0, void 0, function* () {
                return this._twoHopSampler.getTwoHopBuySampleAsync(this._filterTwoHopSourcesByLiquidityChance(sources, tokenPath), tokenPath, makerAmount);
            })));
            return hopResults.filter(h => !!h);
        });
    }
    ...
    _filterSourcesByLiquidityChance(sources, tokenPath) {
        ...
        return sources.filter(source => {
            const score = this._getLiquidityScoreByPathId(source, pathId);
            // score = 1.0 = 100% chance, etc.
            return Math.random() < score;
        });
    }
    _filterTwoHopSourcesByLiquidityChance(sources, tokenPath) {
        const pathIds = [...new Array(tokenPath.length - 1)].map((_, i) => `${tokenPath[i]}:${tokenPath[i + 1]}`);
        return sources.filter(source => {
            // For a two-hop source to be included, it must be eligible for
            // any leg.
    ...
    _scoreLiquidity(source, tokenPath, samples) {
        const pathId = tokenPath.join(':');
        let score = this._getLiquidityScoreByPathId(source, pathId);
    ...
    _getLiquidityScoreByPathId(source, pathId) {
        const score = this._liquidityScores[source][pathId];
        if (score === undefined) {
            return 1.0;
        }
        return score;
