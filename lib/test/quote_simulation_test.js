...
describe('quote_simulation tests', function () { return __awaiter(_this, void 0, void 0, function () {
    ...
    var _a, NULL_ADDRESS, ZERO, ONE, MAKER_TOKEN, TAKER_TOKEN, DEFAULT_MAKER_ASSET_DATA, DEFAULT_TAKER_ASSET_DATA, EPS, GAS_SCHEDULE;
    var _this = this;
        ...
        describe('fillQuoteOrders()', function () {
            describe('single order', function () {
                it('can exactly fill one order', function () {
                    ...
                    var result = quote_simulation_1.fillQuoteOrders(side, fillOrders, fillableInput, ONE, GAS_SCHEDULE);
                    ...
                    contracts_test_utils_1.assertIntegerRoughlyEquals(totalFilledOutput, fillableOutput, EPS);
                ...
                it('can partially fill one simple order', function () {
                    ...
                    var result = quote_simulation_1.fillQuoteOrders(side, fillOrders, inputFillAmount, ONE, GAS_SCHEDULE);
                    var totalFilledInput = result.input.plus(result.inputFee);
                    var totalFilledOutput = result.output.plus(result.outputFee);
                    ...
                    var expectedOutputFilledAmount = inputFillAmount
                        ...
                        .integerValue();
                    contracts_test_utils_1.assertIntegerRoughlyEquals(totalFilledOutput, expectedOutputFilledAmount, EPS);
                    contracts_test_utils_1.expect(result.protocolFee).to.bignumber.eq(1);
                ...
                it('can exactly fill one order with input fees', function () {
                    ...
                    var result = quote_simulation_1.fillQuoteOrders(side, fillOrders, totalFillableInput, ONE, GAS_SCHEDULE);
                    var totalFilledInput = result.input.plus(result.inputFee);
                    var totalFilledOutput = result.output.plus(result.outputFee);
                    contracts_test_utils_1.assertIntegerRoughlyEquals(totalFilledInput, totalFillableInput, EPS);
                    contracts_test_utils_1.assertIntegerRoughlyEquals(totalFilledOutput, fillableOutput, EPS);
                ...
                it('can partially fill one order with input fees', function () {
                    ...
                    var totalFilledInput = result.input.plus(result.inputFee);
                    var totalFilledOutput = result.output.plus(result.outputFee);
                    contracts_test_utils_1.assertIntegerRoughlyEquals(totalFilledInput, inputFillAmount, EPS);
                    contracts_test_utils_1.expect(totalFilledOutput).to.bignumber.lt(fillableOutput);
                    assertEqualRates(result.inputFee.div(result.input), signedInputFeeRate);
                    contracts_test_utils_1.expect(result.protocolFee).to.bignumber.eq(1);
                    contracts_test_utils_1.expect(result.gas).to.lte(fillsCount);
                ...
                it('can exactly fill one order with output fees', function () {
                    ...
                    var totalFilledInput = result.input.plus(result.inputFee);
                    var totalFilledOutput = result.output.plus(result.outputFee);
                    contracts_test_utils_1.assertIntegerRoughlyEquals(totalFilledInput, fillableInput, EPS);
                    contracts_test_utils_1.assertIntegerRoughlyEquals(totalFilledOutput, totalFillableOutput, EPS);
        ...
        describe('simulateBestCaseFill()', function () {
            ...
            it('can fully fill orders with input fees', function () { return __awaiter(_this, void 0, void 0, function () {
                ...
                return __generator(this, function (_a) {
                    ...
                    contracts_test_utils_1.expect(result.protocolFeeAmount).to.bignumber.gt(orders.length);
                    if (side === types_1.MarketOperation.Sell) {
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.takerAssetAmount, fillableInput, EPS);
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.totalTakerAssetAmount, totalFillableInput, EPS);
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.makerAssetAmount, fillableOutput, EPS);
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.totalMakerAssetAmount, fillableOutput, EPS);
                        contracts_test_utils_1.expect(result.makerAssetAmount).to.bignumber.eq(result.totalMakerAssetAmount);
                        contracts_test_utils_1.expect(result.takerFeeMakerAssetAmount).to.bignumber.eq(0);
                    }
                    else {
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.makerAssetAmount, fillableInput, EPS);
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.totalMakerAssetAmount, totalFillableInput, EPS);
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.takerAssetAmount, fillableOutput, EPS);
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.totalTakerAssetAmount, fillableOutput, EPS);
                        contracts_test_utils_1.expect(result.takerAssetAmount).to.bignumber.eq(result.totalTakerAssetAmount);
                        contracts_test_utils_1.expect(result.takerFeeTakerAssetAmount).to.bignumber.eq(0);
                    }
                    return [2 /*return*/];
            ...
            it('can partially fill orders with input fees', function () { return __awaiter(_this, void 0, void 0, function () {
                ...
                return __generator(this, function (_a) {
                    ...
                    contracts_test_utils_1.expect(result.protocolFeeAmount).to.bignumber.gt(0);
                    if (side === types_1.MarketOperation.Sell) {
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.totalTakerAssetAmount, inputFillAmount, EPS);
                        contracts_test_utils_1.expect(result.makerAssetAmount).to.bignumber.lt(fillableOutput);
                        contracts_test_utils_1.expect(result.makerAssetAmount).to.bignumber.eq(result.totalMakerAssetAmount);
                        contracts_test_utils_1.expect(result.takerFeeMakerAssetAmount).to.bignumber.eq(0);
                    }
                    else {
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.totalMakerAssetAmount, inputFillAmount, EPS);
            ...
            it('can fully fill orders with output fees', function () { return __awaiter(_this, void 0, void 0, function () {
                ...
                return __generator(this, function (_a) {
                    ...
                    if (side === types_1.MarketOperation.Sell) {
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.takerAssetAmount, fillableInput, EPS);
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.totalTakerAssetAmount, fillableInput, EPS);
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.makerAssetAmount, fillableOutput, EPS);
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.totalMakerAssetAmount, totalFillableOutput, EPS);
                        contracts_test_utils_1.expect(result.takerAssetAmount).to.bignumber.eq(result.totalTakerAssetAmount);
                        contracts_test_utils_1.expect(result.takerFeeTakerAssetAmount).to.bignumber.eq(0);
                    }
                    else {
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.makerAssetAmount, fillableInput, EPS);
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.totalMakerAssetAmount, fillableInput, EPS);
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.takerAssetAmount, fillableOutput, EPS);
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.totalTakerAssetAmount, totalFillableOutput, EPS);
                        contracts_test_utils_1.expect(result.makerAssetAmount).to.bignumber.eq(result.totalMakerAssetAmount);
                        contracts_test_utils_1.expect(result.takerFeeMakerAssetAmount).to.bignumber.eq(0);
                    }
                    return [2 /*return*/];
            ...
            it('includes order slippage', function () { return __awaiter(_this, void 0, void 0, function () {
                ...
                return __generator(this, function (_a) {
                    ...
                    if (side === types_1.MarketOperation.Sell) {
                        ...
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.totalMakerAssetAmount, slippedOutput);
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.totalTakerAssetAmount, fillableInput);
                    }
                    else {
                        slippedOutput = fillableOutput.times(orderSlippage + 1).integerValue();
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.totalMakerAssetAmount, fillableInput);
                        contracts_test_utils_1.assertIntegerRoughlyEquals(result.totalTakerAssetAmount, slippedOutput);
                    }
                    return [2 /*return*/];
                });
            }); });
