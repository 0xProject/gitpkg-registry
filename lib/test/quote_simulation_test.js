...
const types_1 = require("../src/network/types");
const types_2 = require("../src/types");
const quote_simulation_1 = require("../src/utils/quote_simulation");
// tslint:disable: custom-no-magic-numbers
describe('quote_simulation tests', () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const GAS_SCHEDULE = { [types_1.ERC20BridgeSource.Uniswap]: _.constant(1), [types_1.ERC20BridgeSource.Native]: _.constant(1) };
    ...
    function createQuoteFillOrders(opts = {}) {
        const { fillableInput, fillableOutput, inputFeeRate, outputFeeRate, count, fillsCount, side, type } = Object.assign({ fillableInput: getRandomOrderSize(), fillableOutput: getRandomOrderSize(), inputFeeRate: 0, outputFeeRate: 0, count: 3, fillsCount: 3, side: types_2.MarketOperation.Sell }, opts);
        const _inputFeeRate = side === types_2.MarketOperation.Sell ? inputFeeRate : -inputFeeRate;
        const _outputFeeRate = side === types_2.MarketOperation.Sell ? -outputFeeRate : outputFeeRate;
    ...
    function createQuoteFillOrderOrder(input, output, opts = {}) {
        ...
        const makerAmount = side === types_2.MarketOperation.Sell ? output : input;
        const takerAmount = side === types_2.MarketOperation.Sell ? input : output;
        const fillableMakerAmount = side === types_2.MarketOperation.Sell ? fillableOutput : fillableInput;
        const fillableTakerAmount = side === types_2.MarketOperation.Sell ? fillableInput : fillableOutput;
    ...
    function randomSide() {
        return _.sampleSize(Object.values(types_2.MarketOperation), 1)[0];
    }
    ...
    describe('fillQuoteOrders()', () => {
        describe('single order', () => {
            it('can exactly fill one order', () => {
                ...
                const result = quote_simulation_1.fillQuoteOrders(fillOrders, fillableInput, ONE, GAS_SCHEDULE);
            ...
            it('can partially fill one simple order', () => {
                ...
                const result = quote_simulation_1.fillQuoteOrders(fillOrders, inputFillAmount, ONE, GAS_SCHEDULE);
                const totalFilledInput = result.input.plus(result.inputFee);
                const totalFilledOutput = result.output.plus(result.outputFee);
            ...
            it('can exactly fill one order with input fees', () => {
                ...
                const signedInputFeeRate = side === types_2.MarketOperation.Sell ? inputFeeRate : -inputFeeRate;
                const totalFillableInput = fillableInput.times(signedInputFeeRate + 1).integerValue();
                const result = quote_simulation_1.fillQuoteOrders(fillOrders, totalFillableInput, ONE, GAS_SCHEDULE);
                const totalFilledInput = result.input.plus(result.inputFee);
                const totalFilledOutput = result.output.plus(result.outputFee);
            ...
            it('can exactly fill one order with output fees', () => {
                ...
                const signedOutputFeeRate = side === types_2.MarketOperation.Sell ? -outputFeeRate : outputFeeRate;
            ...
            it('does not charge a protocol fee for rfq orders', () => {
                ...
                contracts_test_utils_1.expect(totalFilledInput).to.bignumber.eq(fillableInput);
                assertRoughlyEquals(totalFilledOutput, fillableOutput);
                contracts_test_utils_1.expect(result.protocolFee).to.bignumber.eq(0);
                contracts_test_utils_1.expect(result.gas).to.eq(fillsCount);
            });
        });
    ...
    function slipOrder(order, orderSlippage, side) {
        const makerScaling = side === types_2.MarketOperation.Sell ? 1 - orderSlippage : 1;
        const takerScaling = side === types_2.MarketOperation.Sell ? 1 : orderSlippage + 1;
        // tslint:disable:next-line no-unnecessary-type-assertion
        const nativeFillData = order.fillData;
        const slippedFillData = {
            order: Object.assign(Object.assign({}, nativeFillData.order), { takerAmount: nativeFillData.order.takerAmount.times(takerScaling), makerAmount: nativeFillData.order.makerAmount.times(makerScaling) }),
            signature: nativeFillData.signature,
            maxTakerTokenFillAmount: nativeFillData.maxTakerTokenFillAmount.times(takerScaling),
        };
        return Object.assign(Object.assign({}, order), { makerAmount: order.makerAmount.times(makerScaling), takerAmount: order.takerAmount.times(takerScaling), fillData: slippedFillData });
    }
    describe('simulateBestCaseFill()', () => {
        it('ignores order slippage', () => __awaiter(void 0, void 0, void 0, function* () {
            const side = randomSide();
            const fillableInput = getRandomOrderSize();
            const fillableOutput = getRandomOrderSize();
            ...
            const result = quote_simulation_1.simulateBestCaseFill({
                orders,
                side,
        ...
        it('can fully fill orders', () => __awaiter(void 0, void 0, void 0, function* () {
            const side = randomSide();
            const fillableInput = getRandomOrderSize();
            const fillableOutput = getRandomOrderSize();
        ...
        it('can partial fill orders', () => __awaiter(void 0, void 0, void 0, function* () {
            const side = randomSide();
            const fillableInput = getRandomOrderSize();
            const fillableOutput = getRandomOrderSize();
            const orders = createQuoteFillOrders({
        ...
        it('can fully fill sell orders with "input" fees', () => __awaiter(void 0, void 0, void 0, function* () {
            const side = types_2.MarketOperation.Sell;
            const fillableInput = getRandomOrderSize();
            const fillableOutput = getRandomOrderSize();
            const inputFeeRate = getRandomFeeRate();
            const orders = createQuoteFillOrders({
        ...
        it('can partially fill sell orders with "input" fees', () => __awaiter(void 0, void 0, void 0, function* () {
            const side = types_2.MarketOperation.Sell;
            const fillableInput = getRandomOrderSize();
            const fillableOutput = getRandomOrderSize();
            const inputFeeRate = getRandomFeeRate();
        ...
        it('can fully fill buy orders with "output" fees', () => __awaiter(void 0, void 0, void 0, function* () {
            const side = types_2.MarketOperation.Buy;
            const fillableInput = getRandomOrderSize();
            const fillableOutput = getRandomOrderSize();
            const outputFeeRate = getRandomFeeRate();
            const orders = createQuoteFillOrders({
        ...
        it('can partially fill buy orders with "output" fees', () => __awaiter(void 0, void 0, void 0, function* () {
            const side = types_2.MarketOperation.Buy;
            const fillableInput = getRandomOrderSize();
            const fillableOutput = getRandomOrderSize();
            const outputFeeRate = getRandomFeeRate();
    ...
    describe('simulateWorstCaseFill()', () => {
        it('includes order slippage', () => __awaiter(void 0, void 0, void 0, function* () {
            const side = randomSide();
            const fillableInput = getRandomOrderSize();
            const fillableOutput = getRandomOrderSize();
            ...
            const result = quote_simulation_1.simulateWorstCaseFill({
                orders,
                side,
                fillAmount: fillableInput,
                gasPrice: ONE,
        ...
        it('expects worse price than the best case, even if orders are unsorted', () => __awaiter(void 0, void 0, void 0, function* () {
            const side = randomSide();
            const fillableInput = getRandomOrderSize();
            const fillableOutput = getRandomOrderSize();
            const orderSlippage = getRandomFeeRate();
            ...
            const bestCase = quote_simulation_1.simulateBestCaseFill({
                orders,
                side,
                fillAmount: fillableInput,
                gasPrice: ONE,
            ...
            const worstCase = quote_simulation_1.simulateWorstCaseFill({
                orders,
                side,
                fillAmount: fillableInput,
                gasPrice: ONE,
