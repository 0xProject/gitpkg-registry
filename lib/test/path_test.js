...
const createFill = (source, index = 0, input = new utils_1.BigNumber(100), output = new utils_1.BigNumber(100)) => 
...
describe('Path', () => {
    it('Adds a fallback', () => {
        ...
        (0, contracts_test_utils_1.expect)(sources).to.deep.eq([types_2.ERC20BridgeSource.Native, types_2.ERC20BridgeSource.Native, types_2.ERC20BridgeSource.Uniswap]);
    });
    it('Adds a fallback with LiquidityProvider', () => {
        ...
        path.addFallback(fallback);
        const sources = path.fills.map(f => f.source);
        (0, contracts_test_utils_1.expect)(sources).to.deep.eq([
    ...
    it('Handles duplicates', () => {
        ...
        path.addFallback(fallback);
        const sources = path.fills.map(f => f.source);
        (0, contracts_test_utils_1.expect)(sources).to.deep.eq([types_2.ERC20BridgeSource.Uniswap, types_2.ERC20BridgeSource.LiquidityProvider]);
    });
    it('Moves Native orders to the front and appends with unused fills', () => {
        ...
        const path = path_1.Path.create(types_1.MarketOperation.Sell, [
            createFill(types_2.ERC20BridgeSource.Uniswap, 0, new utils_1.BigNumber(50)),
            createFill(types_2.ERC20BridgeSource.Native, 0, new utils_1.BigNumber(50)),
        ], targetInput);
        const fallback = path_1.Path.create(types_1.MarketOperation.Sell, [
            createFill(types_2.ERC20BridgeSource.Uniswap, 0, new utils_1.BigNumber(50)),
            createFill(types_2.ERC20BridgeSource.Uniswap, 1, new utils_1.BigNumber(50)),
        ], targetInput);
        path.addFallback(fallback);
        const sources = path.fills.map(f => f.source);
        (0, contracts_test_utils_1.expect)(sources).to.deep.eq([types_2.ERC20BridgeSource.Native, types_2.ERC20BridgeSource.Uniswap, types_2.ERC20BridgeSource.Uniswap]);
    });
});
